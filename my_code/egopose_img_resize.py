...
#         # 크롭된 좌표로 변환
#         if crop_x_start <= x <= crop_x_end and crop_y_start <= y <= crop_y_end:
...
def process_images(trainset_dir):
			...
			for env_folder in os.listdir(set_path):
				...
				if os.path.isdir(cam_down_path):
					...
					if os.path.exists(rgba_folder):
						# rgba 폴더 내의 이미지 순회
						for image_name in tqdm(os.listdir(rgba_folder), desc=f'Processing {set_name}/{env_folder}', leave=False):
							...
							try:
								...
								tqdm.write(f"Error processing {image_name}: {e}")
...
if __name__ == "__main__":
	...
	process_images(args.trainset_dir)
