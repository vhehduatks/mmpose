"""
Cascaded Pose Refinement Head V3 - Lower LR + Finer Milestones

Rationale:
    Cascaded Refinement v1 achieved 42.86mm at epoch 9.
    The refinement MLP may benefit from slower, more careful optimization.

    V1 used LR=5e-4 which may cause overshooting in the refinement path.
    V3 uses LR=2.5e-4 (half) with more gradual milestones.

Changes from v1:
    - LR: 5e-4 -> 2.5e-4 (half)
    - milestones: [4, 7] -> [5, 8] (shifted later)
    - gamma: 0.5 (same)

LR Schedule:
    Epoch 1-5:  LR = 2.5e-4
    Epoch 6-8:  LR = 1.25e-4
    Epoch 9-10: LR = 6.25e-5

Target: sub-42.5mm through finer optimization
"""
...
# Lower LR: 5e-4 -> 2.5e-4
optim_wrapper = dict(
    optimizer=dict(lr=0.00025, type='AdamW'),
)

# Shifted milestones: [4, 7] -> [5, 8]
param_scheduler = [
    dict(
        type='MultiStepLR',
        begin=0,
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_cascaded_refinement_v3'),
...
work_dir = 'work_dirs/HMD_xregopose_cascaded_refinement_v3_full'
