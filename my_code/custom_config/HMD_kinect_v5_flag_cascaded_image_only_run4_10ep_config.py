"""
egodataset_flag_fixed_ver5 - Cascaded Refinement V2b + Image-only - RUN 4 (10 epochs)
- 4th run of HMD_kinect_v5_flag_cascaded_image_only_10ep_config.py.
...
- For paper TABLE V: Cascaded + Image-only column (variance / reproducibility check).
- HMD input: 9-dim, all dims zeroed via dataset use_hmd=False (vision-only).
...
- Head: CustomEgoposeCascadedRefinementHead_enhanced (hmd_info_size=9, use_hmd=False)
- Epochs: 10
"""
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='egodataset_flag_fixed_ver5-cascaded-image-only',
            name='HMD_kinect_v5_flag_cascaded_image_only_run4_10ep',
            tags=['kinect_v5_flag', 'cascaded', 'image_only', 'no_hmd', 'run4', '10ep'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_kinect_v5_flag_cascaded_image_only_run4_10ep'
