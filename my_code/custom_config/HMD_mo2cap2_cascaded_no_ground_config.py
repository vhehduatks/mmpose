"""
Mo2Cap2 Cascaded Refinement - NO Ground Reference (9-dim HMD)

Ablation: Compare cascaded model with/without ground reference.

HMD Info: 9-dim (base only, no ground reference)
Architecture: Two-stage cascaded refinement (same as V2, but without ground info)

This isolates the effect of ground reference on the cascaded model.
"""
...
# Same training config as cascaded V2
train_cfg = dict(
...
# Model: Cascaded head with hmd_info_size=9 (NO ground reference)
model = dict(
    ...
    head=dict(
        ...
        in_channels=2048,
        out_channels=15,
        hmd_info_size=9,  # Base HMD only (no ground reference)
...
# Pipeline WITHOUT ground reference enhancement
train_pipeline = [
    ...
    # NO EnhanceHMDInfo_Mo2Cap2 - uses base 9-dim HMD from dataset
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            name='cascaded_no_ground',
            tags=['mo2cap2', 'cascaded', 'no_ground', 'ablation'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_mo2cap2_cascaded_no_ground'
