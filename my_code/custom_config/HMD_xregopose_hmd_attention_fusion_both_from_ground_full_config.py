"""
HMD Attention Fusion + Both From Ground (12 dims)
...
Key idea:
    ...
    This:     HMD(12) → 3 tokens → CrossAttn(HMD, visual) → fused with GAP → Z[64]

HMD token structure for both_from_ground (12-dim):
    - Token 1: Base HMD info (9-dim) → embedded to d_model
    - Token 2: Head height from ground (1-dim) → embedded to d_model
    - Token 3: Hand heights from ground (2-dim) → embedded to d_model

Hypothesis:
    Cross-attention allows the model to selectively weight visual features
    based on HMD context. For example, when head is at a certain height,
    the model can attend more to lower body regions in the heatmap.
    This is more powerful than simple concatenation.

Expected outcome:
    Better utilization of ground-based height information through
    attention-based feature selection.
    Target: Better than both_from_ground baseline (39.81mm)
"""
...
# Model with HMD Attention Fusion Head
model = dict(
    ...
    head=dict(
        type='CustomEgoposeHMDAttentionFusionHead',
        in_channels=2048,
        out_channels=16,
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_hmd_attention_fusion'),
...
work_dir = 'work_dirs/HMD_xregopose_hmd_attention_fusion_both_from_ground_full'
