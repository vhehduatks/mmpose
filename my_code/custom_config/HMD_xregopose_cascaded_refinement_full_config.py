"""
Cascaded Pose Refinement Head - Full Dataset Training

Two-stage architecture:
  Stage 1: Baseline (Deconv → Heatmap → Encoder(GAP+HMD) → Z → PoseDecoder)
  Stage 2: Kinematic-Aware Refinement (GridSample + KinFeatures + MLP → Δpose)

Key idea: Don't replace the Baseline — build on top of it.
  - Stage 1 produces coarse pose (proven 41.37mm Baseline)
  - Stage 2 refines using per-joint spatial features + kinematic chain prior
  - Residual correction: Refined = Coarse + Δpose

Additional losses for Stage 2:
    - pose_l2norm_refined (w=1.0): L2 on refined pose
    - bone_length (w=0.5): Per-limb bone length matching
    - symmetry (w=0.1): Left-right limb symmetry
"""
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_cascaded_refinement'),
...
work_dir = 'work_dirs/HMD_xregopose_cascaded_refinement_full'
