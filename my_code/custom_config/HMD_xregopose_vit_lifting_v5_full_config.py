"""
ViT-Style Lifting Head v5 - Hybrid Attention (Full Training)
...
Architecture:
    ...
    Spatial Tokens [64, D]
         ↓
    Cross-Attention (J → S) × 1   [16×64 attention]
         ↓
    Self-Attention (J → J) × 2    [16×16 attention]
         ↓
    Joint Tokens [16, D]
         ├── Heatmap Decoder (grad scale 0.1)
         ↓
    HMD Cross-Attention           [16×3 attention]
         ↓
    3D Pose Head → [16, 3]

Expected improvements:
- 더 focused attention으로 관절별 위치 학습 향상
- Skeleton 관계 학습 분리
- Heatmap gradient 감소로 3D 학습 안정화
"""
...
# =============================================================================
# LR Schedule - CosineAnnealingLR (from v4)
# =============================================================================
param_scheduler = [
    # Warmup (iter-based)
    dict(
...
# =============================================================================
# Model - ViT-Style Lifting v5 (Hybrid Attention)
# =============================================================================
model = dict(
    ...
    head=dict(
        type='CustomEgoposeViTLiftingHeadV5',
        in_channels=2048,
        out_channels=16,
        decoder=codec,
        # v5 Hybrid Attention params
        embed_dim=256,
        num_heads=8,
        num_cross_layers=1,   # Cross-Attention (J→S) layers
        num_self_layers=2,    # Self-Attention (J→J) layers
        dropout=0.1,
        heatmap_size=47,
        use_hmd=True,
        use_heatmap_recon=True,
        heatmap_grad_scale=0.1,  # KEY: Gradient scaling for heatmap
        # Losses
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_vit_lifting_v5'),
...
work_dir = 'work_dirs/HMD_xregopose_vit_lifting_v5_full'
