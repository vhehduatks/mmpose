"""
Cascaded Pose Refinement Head - 20 Epochs Extended Training

Rationale:
    Cascaded Refinement v1 achieved 42.86mm at epoch 9, still improving.
    ViT Lifting V6 20ep showed extended training can help (-1.70mm vs 10ep).

    This experiment extends training to 20 epochs with adjusted LR schedule
    to allow the refinement MLP more time to learn meaningful corrections.

Changes from v1 (10 epochs):
    - max_epochs: 10 -> 20
    - MultiStepLR milestones: [4, 7] -> [8, 14] (scaled 2x)
    - Keep same LR=5e-4 and gamma=0.5

LR Schedule:
    Epoch 1-8:   LR = 5e-4
    Epoch 9-14:  LR = 2.5e-4
    Epoch 15-20: LR = 1.25e-4

Target: sub-42mm (vs v1's 42.86mm)
"""
...
# Scaled milestones for 20 epochs: [4,7] -> [8,14]
param_scheduler = [
    dict(
        type='MultiStepLR',
        begin=0,
...
model = dict(
    ...
    head=dict(
        type='CustomEgoposeCascadedRefinementHead',
        in_channels=2048,
        out_channels=16,
        decoder=codec,
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_cascaded_refinement_20ep'),
...
work_dir = 'work_dirs/HMD_xregopose_cascaded_refinement_20ep_full'
