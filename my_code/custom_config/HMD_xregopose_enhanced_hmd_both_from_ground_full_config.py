"""
Enhanced HMD Info - Both From Ground (12 dims)

Rationale:
    Most complete ground-based information for HMD deployment.
    Provides all tracked device heights from estimated ground:
    - head_from_ground: HMD headset height from floor
    - left_hand_from_ground: Left controller height from floor
    - right_hand_from_ground: Right controller height from floor

    This is the most comprehensive mode that uses all available ground
    reference information from HMD tracking.

Changes from baseline (41.37mm):
    - Add EnhanceHMDInfo transform with mode='both_from_ground'
    - Use CustomxRegoposeBaselinel1_enhanced_hmd head with hmd_info_size=12
    - HMD info: 9 base dims + 3 (head + left_hand + right_hand from ground) = 12 dims

Comparison:
    - head_from_ground (10 dims): Only head height
    - hand_from_ground (11 dims): Only hand heights
    - ground_reference (11 dims): Head height + head_torso_dist
    - both_from_ground (12 dims): Head + both hand heights (THIS CONFIG)

Hypothesis:
    Combining all ground references (head + both hands) may provide the most
    complete spatial information for accurate 3D pose estimation.
    The ground_reference mode (36.28mm) uses head + body scale info.
    This mode uses head + hand heights which are directly measurable from HMD.

Target: Potentially better than ground_reference (36.28mm) by using more
        directly measurable features from HMD tracking.
"""
...
train_pipeline = [
    ...
    dict(type='EnhanceHMDInfo', mode='both_from_ground'),  # Head + both hands from ground
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_enhanced_hmd_both_from_ground'),
...
work_dir = 'work_dirs/HMD_xregopose_enhanced_hmd_both_from_ground_full'
