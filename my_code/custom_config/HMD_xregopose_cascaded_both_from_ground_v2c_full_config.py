"""
Cascaded Refinement + Both From Ground V2c (EfficientDecoder + Loss Weight Tuning)

Optimizations over V1 (37.88mm):
  1. EfficientHeatmapDecoder: 40M -> 1.35M params
  2. Loss Weight Tuning:
     - loss_heatmap_recon: 500 -> 250 (reduce 2D supervision, focus on 3D)
     - loss_bone_length: 0.5 -> 0.75 (stronger structural constraint)
     - loss_symmetry: 0.1 -> 0.2 (stronger body symmetry)

Hypothesis: Reducing heatmap reconstruction weight lets the model focus
more on 3D pose quality. Stronger structural losses improve anatomical
consistency.

Target: Improve over V1 (37.88mm) with better loss balance.
"""
...
# Model with EfficientHeatmapDecoder + Tuned Loss Weights
model = dict(
    ...
    head=dict(
        ...
        # V2 Optimization: EfficientHeatmapDecoder
        ...
        # V2c: Reduced heatmap_recon weight (focus more on 3D)
        loss_heatmap_recon=dict(
            loss_weight=250,  # V1: 500 -> V2c: 250
            type='KeypointMSELoss',
            use_target_weight=False
        ),
        ...
        # Stage 2 losses (refinement) - V2c: Increased structural loss weights
        loss_pose_l2norm_refined=dict(loss_weight=1.0, type='pose_l2norm'),
        loss_bone_length=dict(loss_weight=0.75, type='bone_length_loss'),  # V1: 0.5 -> V2c: 0.75
        loss_symmetry=dict(loss_weight=0.2, type='symmetry_loss'),  # V1: 0.1 -> V2c: 0.2
    ),
    test_cfg=dict(
        flip_test=False,
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_cascaded_both_from_ground_v2c'),
...
work_dir = 'work_dirs/HMD_xregopose_cascaded_both_from_ground_v2c_full'
