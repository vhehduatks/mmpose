"""
Cascaded Refinement + Both From Ground V2 (EfficientHeatmapDecoder)

Optimization over V1 (37.88mm):
  - EfficientHeatmapDecoder: 40M -> 1.35M params (Conv-based instead of Linear)
  - Uses transposed convolutions instead of massive linear layer
  - Preserves spatial structure better

Key architecture:
  - Stage 1: ResNet -> Heatmap -> Encoder -> Z -> PoseDecoder -> Coarse 3D
  - Stage 2: GridSample + Kinematic + HMD -> RefinementMLP -> Refined 3D
  - Enhanced HMD: 9 base + 3 ground heights = 12 dims (HMD-deployable)

Target: Maintain ~37.88mm while reducing parameters significantly.
"""
...
# Model with Enhanced Cascaded Refinement Head (12-dim HMD) + EfficientHeatmapDecoder
model = dict(
    ...
    head=dict(
        ...
        # V2 Optimization: EfficientHeatmapDecoder (1.35M vs 40M params)
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_cascaded_both_from_ground_v2'),
...
work_dir = 'work_dirs/HMD_xregopose_cascaded_both_from_ground_v2_full'
