"""
Attention-based Lifting Head v2 - SMOKE TEST Config
...
# Optimizer with Gradient Clipping (same as v2)
optim_wrapper = dict(
    optimizer=dict(lr=0.0005, type='AdamW', weight_decay=0.01),
    clip_grad=dict(max_norm=1.0, norm_type=2),
)

# LR Schedule: Simplified for 1 epoch
param_scheduler = [
    dict(
        type='LinearLR',
        start_factor=0.1,
...
default_hooks = dict(
    ...
    visualization=dict(
        enable=False,  # Disabled for smoke test
        interval=100,
        kpt_thr=0.3,
...
dataset_train = dict(
    ...
    data_mode=data_mode,
    data_root=data_root,
    cache_file=cache_file_train,  # Small: train_small_1k.h5
    rebuild_cache=False,
...
dataset_val = dict(
    ...
    data_mode=data_mode,
    data_root=data_root,
    cache_file=cache_file_val,  # Small: val_small_500.h5
    rebuild_cache=False,
    use_cached_images=True,
    filter_cfg=dict(filter_empty_gt=False, min_size=32),
...
# =============================================================================
# Visualization - Local only (no wandb for smoke test)
# =============================================================================
vis_backends = [
...
work_dir = 'work_dirs/HMD_xregopose_attention_lifting_v2_small'
