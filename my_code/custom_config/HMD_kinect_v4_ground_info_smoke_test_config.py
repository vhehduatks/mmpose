"""
Kinect v4 Ground Info Smoke Test Config (12-dim HMD, with ground info)
- Dataset: KinectEgoposeDataset (Azure Kinect 32→16 joint mapping)
- Backbone: ResNet-101 pretrained on COCO
- Head: CustomxRegoposeBaselinel1_enhanced_hmd (hmd_info_size=12)
- Ground Info: ON (both_from_ground → 12-dim HMD info)
  Uses Unity Y-axis: hmd_pos_y, left_pos_y, right_pos_y from synced_data.csv
- Epochs: 2 (smoke test only)
- No checkpoint saving, wandb enabled
"""
...
_max_epochs = 5  # Change to 3, 5, or 10 for comparison runs
...
param_scheduler = [
    dict(
        ...
        milestones=[_max_epochs // 2] if _max_epochs > 2 else [1],
        gamma=0.5,
        by_epoch=True
    ),
]
...
# =============================================================================
# Model - Enhanced HMD (12-dim, with ground info)
# =============================================================================
model = dict(
    ...
    head=dict(
        decoder=codec,
        in_channels=2048,
        hmd_info_size=12,  # 9 base + 3 ground heights
        loss=dict(
            loss_weight=1000,
        ...
        out_channels=16,
        type='CustomxRegoposeBaselinel1_enhanced_hmd'  # hmd_info_size=12
    ),
    test_cfg=dict(
        flip_test=False,
...
# =============================================================================
# Dataset Config - WITH ground info (ground_info_mode='both_from_ground' → 12-dim)
# =============================================================================
data_mode = 'topdown'
...
dataset_train = dict(
    ...
    ground_info_mode='both_from_ground',  # Enhanced 12-dim HMD info
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='kinect-ground-info-12dim',
            name=f'kinect_v4_ground_info_12dim_{_max_epochs}ep',
            tags=['kinect', 'ground_info', '12dim_hmd', 'smoke_test'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_kinect_v4_ground_info_smoke_test'
