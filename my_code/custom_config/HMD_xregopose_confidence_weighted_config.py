"""
XR EgoPose Training Config with Confidence-Weighted HMD Fusion

Key Difference from Baseline:
- Uses heatmap confidence to dynamically weight HMD vs visual features
- Low heatmap confidence (occluded joints) -> rely more on HMD information
- High heatmap confidence (visible joints) -> rely more on visual features

Architecture:
    confidence = sigmoid(heatmap.max())       # per-joint [0, 1]
    hmd_weight = 1 - confidence               # inverse relationship
    z_fused = z_visual * confidence + z_hmd * hmd_weight

Expected: Better handling of occluded joints in egocentric view
"""
...
# =============================================================================
# Model Architecture - Confidence-Weighted HMD Fusion
# =============================================================================
model = dict(
    ...
    head=dict(
        ...
        out_channels=16,
        # Use the new confidence-weighted head
        type='ConfidenceWeightedHMDHead',
        # Pose decoder config (same as improved baseline)
...
# =============================================================================
# Data Pipeline (with PhotometricDistortion)
# =============================================================================
train_pipeline = [
    dict(type='LoadImage'),
    dict(padding=1.0, type='GetBBoxCenterScale'),
    dict(input_size=(256, 256), type='TopdownAffine'),
    # PhotometricDistortion for color augmentation (safe for 3D)
    dict(
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_confidence_weighted'),
...
work_dir = 'work_dirs/HMD_xregopose_confidence_weighted'
