"""Task 9 secondary ablation — Ours-T k=3, DIRECTION-ONLY rays (de-rotated 2D;
Plücker moment/origin term zeroed) — isolates what camera-translation adds.
Base: Task 9.3 — Ours-T k=3: like the pilot but the TRAIN 2D history comes from
the fold-swapped stage-1 caches (kp2d_ours, k=3 participant folds — each
session's history predicted by a checkpoint that never saw its participant),
so train-time ray noise matches test time. Val history stays the full-ckpt
cache (kp2d_ours_pilot), which IS the exact test-time semantics for a frozen
stage 1.
...
kp2d_cache_name = 'kp2d_ours'          # train (fold-swapped)
kp2d_cache_name_val = 'kp2d_ours_pilot' # val (= test-time exact)
...
dataset_val = dict(
    ...
    frame_export_root=frame_export_root,
    kp2d_cache_name=kp2d_cache_name_val,
    history=20,
    ground_info_mode='both_from_ground',
    filter_cfg=dict(filter_empty_gt=False, min_size=32),
    use_2d_visible=True,
