"""
egodataset_flag_fixed_ver5 - Cascaded Stage 1 Only + No-HMD (vision-only) (10 epochs)
- For paper TABLE V: Single-stage + Image-only row (apples-to-apples with Single-stage + LHF row)
- Uses CascadedRefinementHead_enhanced with use_refinement=False (Stage 2 disabled)
- HMD input: zeroed via dataset use_hmd=False (vision-only ablation)
...
dataset_train = dict(
    ...
    ground_info_mode=None,  # No ground info (9-dim shape, but values zeroed)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='egodataset_flag_fixed_ver5-cascaded-stage1-only-no-hmd',
            name='HMD_kinect_v5_flag_cascaded_stage1_only_no_hmd_10ep',
            tags=['kinect_v5_flag', 'cascaded_stage1_only', 'no_hmd', 'image_only', '10ep'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_kinect_v5_flag_cascaded_stage1_only_no_hmd_10ep'
