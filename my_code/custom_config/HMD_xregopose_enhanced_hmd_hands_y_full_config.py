"""
Enhanced HMD Info - Hands Y (11 dims)

Rationale:
    keypoint3d is root-relative (head at origin), so head_y = 0 always.
    Adding hands Y relative to head tells the model vertical position
    of hands (above/below head level).

Changes from baseline (41.37mm):
    - Add EnhanceHMDInfo transform with mode='hands_y'
    - Use CustomxRegoposeBaselinel1_enhanced_hmd head with hmd_info_size=11
    - HMD info: 9 base dims + 2 (right_hand_y, left_hand_y) = 11 dims

Target: sub-41mm through hands height context
"""
...
model = dict(
    ...
    head=dict(
        ...
        in_channels=2048,
        out_channels=16,
        hmd_info_size=11,  # 9 base + 2 (right_hand_y, left_hand_y)
        decoder=codec,
        loss=dict(
            loss_weight=1000,
...
train_pipeline = [
    ...
    dict(type='EnhanceHMDInfo', mode='hands_y'),  # NEW: Add head and hands Y
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_enhanced_hmd_hands_y'),
...
work_dir = 'work_dirs/HMD_xregopose_enhanced_hmd_hands_y_full'
