"""
Cascaded Refinement + Both From Ground V2b (EfficientDecoder + Extended Training)

Optimizations over V1 (37.88mm):
  1. EfficientHeatmapDecoder: 40M -> 1.35M params
  2. Extended Training:
     - max_epochs: 10 -> 20 (more convergence time)
     - milestones: [4,7] -> [8,14] (adjusted for 20 epochs)

Hypothesis: 10 epochs may not be enough for full convergence.
Extended training could improve both Stage 1 and Stage 2.

Target: Improve over V1 (37.88mm) with longer training.
"""
...
# V2b: Extended training
train_cfg = dict(
    type='EpochBasedTrainLoop',
    max_epochs=20,  # V1: 10 -> V2b: 20
    val_interval=1,
)
val_cfg = dict()
test_cfg = None
...
param_scheduler = [
    dict(
        ...
        milestones=[8, 14],  # V1: [4,7] -> V2b: [8,14]
        gamma=0.5,
        by_epoch=True
    ),
]
...
# Model with EfficientHeatmapDecoder (same as V2, training extended)
model = dict(
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_cascaded_both_from_ground_v2b'),
...
work_dir = 'work_dirs/HMD_xregopose_cascaded_both_from_ground_v2b_full'
