"""
Skeleton Graph Attention Network - Full Training Config

Key difference from Attention Lifting:
- JointSelfAttention → SkeletonGraphAttention
...
LR Schedule: v1-style (proven best for attention-based heads)
- LR=0.0005 (optimal for attention)
- MultiStepLR [4, 7]
- Gradient clipping (max_norm=1.0)
- No warmup (warmup hurts attention learning)

Expected improvements over Attention Lifting:
- Better structural consistency (skeleton prior)
- More stable training (reduced attention noise)
- Improved limb length preservation
"""
...
# =============================================================================
# Optimizer: v1's LR + Gradient Clipping (proven best for attention)
# =============================================================================
optim_wrapper = dict(
...
model = dict(
    ...
    head=dict(
        type='CustomEgoposeSkeletonGATHead',
        in_channels=2048,
        out_channels=16,
        decoder=codec,
        # GAT params
        joint_dim=64,
        num_heads=4,
        num_gat_layers=2,  # Same as Attention Lifting's num_self_attn_layers
        dropout=0.1,
        detach_2d_coords=True,
        use_second_order=True,  # Include 2-hop neighbors for richer structure
        # Losses (same as Attention Lifting v1)
        loss=dict(
            loss_weight=1000,
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_skeleton_gat'),
...
work_dir = 'work_dirs/HMD_xregopose_skeleton_gat_full'
