"""
Enhanced HMD Info - Option 3: Torso Reference (11 dims)

Rationale:
    Adding torso reference point (pelvis average) Y and head-to-torso distance
    provides body scale information that can help the model understand
    the relationship between upper and lower body.

Changes from baseline (41.37mm):
    - Add EnhanceHMDInfo transform with mode='torso_reference'
    - Use CustomxRegoposeBaselinel1_enhanced_hmd head with hmd_info_size=11
    - HMD info: 9 base dims + 2 (torso_y, head_torso_dist) = 11 dims

Target: sub-41mm through body scale context
"""
...
model = dict(
    ...
    head=dict(
        ...
        in_channels=2048,
        out_channels=16,
        hmd_info_size=11,  # 9 base + 2 (torso_y, head_torso_dist)
        decoder=codec,
        loss=dict(
            loss_weight=1000,
...
train_pipeline = [
    ...
    dict(type='EnhanceHMDInfo', mode='torso_reference'),  # NEW: Add torso reference
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_enhanced_hmd_torso_ref'),
...
work_dir = 'work_dirs/HMD_xregopose_enhanced_hmd_torso_ref_full'
