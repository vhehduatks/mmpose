"""
egodataset_flag_fixed_ver5 - Cascaded Stage 1 Only + LHF only (10 epochs)
- For paper TABLE V: Single-stage + LHF column (re-run, parallel naming with
  *_image_only / *_gbh_only / *_lhf_gbh siblings).
- Apples-to-apples sibling of HMD_kinect_v5_flag_cascaded_stage1_only_gbh_only_10ep_config.py:
  ...
  schedule, only the HMD input differs — LHF dims kept, no GBH heights.
- HMD input: 9-dim, dims 0-8 (LHF) active; no ground info appended.
...
- Head: CustomEgoposeCascadedRefinementHead_enhanced (Stage 1 only, hmd_info_size=9)
- Epochs: 10
"""
...
model = dict(
    ...
    head=dict(
        ...
        in_channels=2048,
        out_channels=16,
        hmd_info_size=9,              # LHF only (no ground-based heights)
...
dataset_train = dict(
    ...
    ground_info_mode=None,  # No ground info (9-dim LHF only)
    use_hmd=True,            # LHF active
    use_lhf=True,            # Keep LHF dims
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='egodataset_flag_fixed_ver5-cascaded-stage1-only-lhf-only',
            name='HMD_kinect_v5_flag_cascaded_stage1_only_lhf_only_10ep',
            tags=['kinect_v5_flag', 'cascaded_stage1_only', 'lhf_only', '10ep'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_kinect_v5_flag_cascaded_stage1_only_lhf_only_10ep'
