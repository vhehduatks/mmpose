"""
Kinect v4 No-HMD Smoke Test Config (vision-only, zeroed HMD)
- Dataset: KinectEgoposeDataset (Azure Kinect 32→16 joint mapping)
- Backbone: ResNet-101 pretrained on COCO
- Head: CustomxRegoposeBaselinel1 (hmd_info_size=9, but input is zeros)
- HMD: OFF (use_hmd=False → all-zero HMD info for vision-only ablation)
...
# =============================================================================
# Model - No HMD (vision-only, same head but HMD input is zeros)
# =============================================================================
model = dict(
    ...
    head=dict(
        ...
        out_channels=16,
        type='CustomxRegoposeBaselinel1'  # hmd_info_size=9 (zeros input)
    ),
    test_cfg=dict(
        flip_test=False,
...
# =============================================================================
# Dataset Config - NO HMD (use_hmd=False → zeros for vision-only ablation)
# =============================================================================
data_mode = 'topdown'
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='kinect-no-hmd-vision-only',
            name=f'kinect_v4_no_hmd_{_max_epochs}ep',
            tags=['kinect', 'no_hmd', 'vision_only', 'smoke_test'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_kinect_v4_no_hmd_smoke_test'
