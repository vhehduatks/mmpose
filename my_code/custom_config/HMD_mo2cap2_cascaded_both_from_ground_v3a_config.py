"""
Mo2Cap2 Cascaded Refinement + BOTH FROM GROUND V3a (Asymmetric Loss + Lower LR)

V3 Asymmetric Loss with lower learning rate.

Changes from V3:
  - lr: 0.00025 → 0.000125 (half)

Base V3 changes (from V2):
  - loss_pose_l2norm: 1.0 → 0.3 (lower weight for coarse Stage 1)
  - loss_pose_l2norm_refined: 1.0 (full weight for refined Stage 2)
"""
...
# V3a: Lower learning rate (0.000125)
optim_wrapper = dict(
...
model = dict(
    ...
    head=dict(
        ...
        loss_pose_l2norm=dict(loss_weight=0.3, type='pose_l2norm'),  # Asymmetric
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            name='cascaded_v3a_asymmetric_lr0.000125',
            tags=['mo2cap2', 'cascaded', 'asymmetric_loss', 'v3a', 'lr_ablation'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_mo2cap2_cascaded_both_from_ground_v3a'
