"""
Mo2Cap2 Cascaded Refinement + BOTH FROM GROUND V3 (Asymmetric Loss Weights)

Hypothesis: Equal weights for coarse and refined pose losses cause gradient competition.

Changes from V2:
  - loss_pose_l2norm: 1.0 → 0.3 (lower weight for coarse Stage 1)
  - loss_pose_l2norm_refined: 1.0 (unchanged, full weight for refined Stage 2)

Expected outcome: Better Stage 2 learning without destabilizing Stage 1.

Reference: MO2CAP2_CASCADED_ANALYSIS.md
"""
...
# Model: Asymmetric loss weights for coarse vs refined pose
model = dict(
    ...
    head=dict(
        ...
        loss_pose_l2norm=dict(loss_weight=0.3, type='pose_l2norm'),  # V3: 1.0 → 0.3 (lower for coarse)
        loss_hmd=dict(type='MSELoss'),
        # Stage 2 losses
        loss_pose_l2norm_refined=dict(loss_weight=1.0, type='pose_l2norm'),  # Full weight for refined
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            name='cascaded_BOTH_from_ground_v3_asymmetric_loss',
            tags=['mo2cap2', 'cascaded', 'both_from_ground', 'asymmetric_loss', 'v3'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_mo2cap2_cascaded_both_from_ground_v3'
