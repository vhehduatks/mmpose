"""
Cascaded Refinement + Both From Ground (12 dims)

Combines two promising approaches:
  1. Cascaded Refinement Head (41.60mm) - Two-stage refinement architecture
  2. both_from_ground enhanced HMD (39.81mm) - Best HMD-deployable result

Key differences from standalone Cascaded Refinement:
  - EnhanceHMDInfo transform with mode='both_from_ground'
  - CustomEgoposeCascadedRefinementHead_enhanced with hmd_info_size=12
  - HMD info: 9 base + 3 (head + left_hand + right_hand from ground) = 12 dims
  - Enhanced HMD info also feeds into Stage 2 refinement

Hypothesis:
    Cascaded Refinement can better utilize the ground-based height information
    through its per-joint spatial features and kinematic chain encoding.
    Stage 2 refinement can leverage the ground reference for more accurate
    depth estimation of lower body joints.

Expected outcome:
    Better than both standalone approaches:
    - Cascaded alone: 41.60mm
    - both_from_ground alone: 39.81mm
    Target: sub-39mm combining refinement + ground reference
"""
...
# Model with Enhanced Cascaded Refinement Head (12-dim HMD)
model = dict(
...
train_pipeline = [
    ...
    dict(type='EnhanceHMDInfo', mode='both_from_ground'),  # Ground-based heights
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_cascaded_both_from_ground'),
...
work_dir = 'work_dirs/HMD_xregopose_cascaded_both_from_ground_full'
