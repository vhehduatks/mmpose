"""
Single COCO Backbone Config - Full Dataset Training
- Backbone: ResNet-101 pretrained on COCO
- Train: Full train cache (~210k samples)
- Val: Test cache (~60k samples) for per-epoch evaluation
- 10 epochs
"""
...
if IS_WINDOWS:
    ...
    cache_file_val = r'F:\egodataset_cache\h5cache\test_cache_with_images.h5'  # Use test set for validation
else:
...
val_cfg = dict()
test_cfg = None  # No separate test loop, using val for test set evaluation
...
dataset_val = dict(
    ...
    data_mode=data_mode,
    data_root=data_root,
    cache_file=cache_file_val,  # Using TEST cache for validation
    rebuild_cache=False,
    use_cached_images=True,
    filter_cfg=dict(filter_empty_gt=False, min_size=32),
...
if IS_WINDOWS:
    ...
    _batch_size = 48  # Adjust based on GPU memory
else:
    _num_workers = 6
...
val_evaluator = dict(
    ...
    use_action=True  # Report per-action metrics on test set
)
...
work_dir = 'work_dirs/HMD_xregopose_single_coco_full'
