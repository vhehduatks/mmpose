"""
Attention-based Lifting Head v2 - Full Training (Stabilized)
- LR Warmup (2 epochs) + CosineAnnealing (smoother decay)
- Gradient Clipping (max_norm=1.0)
- Comparison target: v1 (45.43mm at epoch 7)

Changes from v1:
1. LinearLR warmup (0.1x → 1x over 2 epochs)
2. CosineAnnealingLR instead of MultiStepLR
...
# =============================================================================
# Training Config - Stabilized (10 epochs)
# =============================================================================
...
optim_wrapper = dict(
    ...
    clip_grad=dict(max_norm=1.0, norm_type=2),  # Gradient clipping
)

# LR Schedule: Warmup + CosineAnnealing
param_scheduler = [
    # Phase 1: Linear Warmup (epoch 0-2)
    dict(
        type='LinearLR',
        start_factor=0.1,  # Start at 0.1x LR
        by_epoch=True,
        begin=0,
        end=2,
    ),
    # Phase 2: Cosine Annealing (epoch 2-10)
    dict(
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_attention_lifting_v2'),
...
work_dir = 'work_dirs/HMD_xregopose_attention_lifting_v2_full'
