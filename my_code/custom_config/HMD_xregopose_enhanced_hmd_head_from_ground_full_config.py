"""
Enhanced HMD Info - Head From Ground (10 dims)

Rationale:
    Simplest ground-based feature - only head height from estimated ground.
    This is exactly what HMD tracking systems measure: distance from headset to floor.

    Compared to ground_reference (11 dims = head_from_ground + head_torso_dist),
    this uses only 1 additional feature instead of 2.

Changes from baseline (41.37mm):
    - Add EnhanceHMDInfo transform with mode='head_from_ground'
    - Use CustomxRegoposeBaselinel1_enhanced_hmd head with hmd_info_size=10
    - HMD info: 9 base dims + 1 (head_from_ground) = 10 dims

Hypothesis:
    Head height alone may be sufficient for lower body estimation.
    The ground_reference mode (36.28mm) uses head_torso_dist as second feature,
    but that requires knowing torso position which isn't directly measurable.

Target: Compare with ground_reference (36.28mm) to see if head_torso_dist is necessary.
"""
...
# Model with enhanced HMD head (10 dims)
model = dict(
...
train_pipeline = [
    ...
    dict(type='EnhanceHMDInfo', mode='head_from_ground'),  # Only head height
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_enhanced_hmd_head_from_ground'),
...
work_dir = 'work_dirs/HMD_xregopose_enhanced_hmd_head_from_ground_full'
