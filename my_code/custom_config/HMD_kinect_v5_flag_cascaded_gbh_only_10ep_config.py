"""
egodataset_flag_fixed_ver5 - Cascaded Refinement V2b + GBH-only (10 epochs)
- For paper TABLE V: Cascaded + GBH-only column
- Apples-to-apples with the 70.97mm Cascaded + LHF row:
  same head (CascadedRefinementHead_enhanced, use_refinement default True),
  same schedule, only the HMD input differs — LHF dims zeroed, GBH dims kept.
- HMD input: 12-dim, dims 0-8 (LHF) zeroed via dataset use_lhf=False;
  dims 9-11 (GBH ground heights) preserved.
...
model = dict(
    ...
    head=dict(
        ...
        in_channels=2048,
        out_channels=16,
        hmd_info_size=12,             # 9 LHF (zeroed) + 3 GBH ground heights
...
dataset_train = dict(
    ...
    use_lhf=False,                          # GBH-only: zero the 9 LHF dims
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='egodataset_flag_fixed_ver5-cascaded-gbh-only',
            name='HMD_kinect_v5_flag_cascaded_gbh_only_10ep',
            tags=['kinect_v5_flag', 'cascaded', 'gbh_only', '10ep'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_kinect_v5_flag_cascaded_gbh_only_10ep'
