"""Task 37 D1 — structural losses OFF (bone/cosine/limb/symmetry weights -> 0.0).
Derived from HMD_xregopose_cascaded_both_from_ground_v3_full_config.py; ONLY the four loss weights and work_dir/wandb names change.
"""
"""
...
model = dict(
    ...
    head=dict(
        ...
        loss_cosine_similarity=dict(loss_weight=0.0, type='cosine_similarity'),
        loss_heatmap_recon=dict(
            loss_weight=500,
            type='KeypointMSELoss',
            use_target_weight=False
        ),
        loss_limb_length=dict(loss_weight=0.0, type='limb_length'),
        ...
        loss_bone_length=dict(loss_weight=0.0, type='bone_length_loss'),
        loss_symmetry=dict(loss_weight=0.0, type='symmetry_loss'),
    ),
    test_cfg=dict(
        flip_test=False,
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_t37_noslosses'),
...
work_dir = 'work_dirs/HMD_xregopose_cascaded_gbh_noslosses'
