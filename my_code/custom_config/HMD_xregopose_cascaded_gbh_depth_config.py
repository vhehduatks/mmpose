"""
Cascaded + GBH + Depth Enhancement on xR-EgoPose.

Uses depth map (from H5 cache) as additional input to Stage 2 refinement.
Depth is processed through a small CNN encoder, then grid-sampled at
predicted 2D joint locations to provide per-joint depth features.

- Dataset: H5CachedEgoposeDataset with depth (test_cache_with_images_depth_v2.h5)
- Head: CustomEgoposeCascadedRefinementHead_depth
- HMD: 12-dim (GBH)
- Depth: 256x256 uint8 → DepthEncoder(32ch) → grid-sample → 16-dim per joint
"""
...
if IS_WINDOWS:
    ...
    cache_file_train = r'F:\egodataset_cache\h5cache\train_cache_with_images_depth_v2.h5'
    cache_file_val = r'F:\egodataset_cache\h5cache\test_cache_with_images_depth_v2.h5'
else:
    data_root = '/mnt/dataset_vol/xregopose_depth_h5'
    pretrained_coco = '/mnt/dataset_vol/pretrained/coco_pose_resnet_101_256x192.pth.tar'
    cache_file_train = '/mnt/dataset_vol/xregopose_depth_h5/train_cache_with_images_depth_v2.h5'
    cache_file_val = '/mnt/dataset_vol/xregopose_depth_h5/test_cache_with_images_depth_v2.h5'
...
model = dict(
    ...
    head=dict(
        type='CustomEgoposeCascadedRefinementHead_depth',
        in_channels=2048,
        out_channels=16,
        hmd_info_size=12,
        heatmap_decoder_type='efficient',
        decoder=codec,
        # Depth-specific params
        use_depth=True,
        depth_feat_dim=16,
        depth_encoder_channels=32,
        # Stage 1 losses
        loss=dict(
            loss_weight=1000,
...
# Pipeline: load image + depth, then process
train_pipeline = [
    dict(type='LoadImageFromH5Cache'),
    dict(type='LoadDepthFromH5Cache'),
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_cascaded_gbh_depth'),
...
work_dir = 'work_dirs/HMD_xregopose_cascaded_gbh_depth'
