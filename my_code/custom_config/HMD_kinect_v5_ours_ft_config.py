"""Task 12.2 — Ours-Ft: per-joint feature-history tokens into stage 2.

Fine-tunes stage 2 + the feature-history branch from the headline checkpoint
(stage 1 + backbone frozen via FreezeStage1Hook); the per-sample history is
the frozen model's own sampled per-joint backbone features (activation cache
feat_ours_pilot, lazily mmap-sliced). Controls via history_mode:
'replicate' (capacity) and 'coords' (any-history).
...
Launch (mmpose env; PYTHONPATH so custom_imports resolves):
    ...
    /home/.conda/envs/mmpose/bin/python tools/train.py \
        my_code/custom_config/HMD_kinect_v5_ours_ft_config.py \
        --work-dir work_dirs/ours_ft

GATE 12b: a val run of this config with the headline checkpoint and
zero_f_temp=True on the head must reproduce 64.357/60.909/67.805 exactly
(the f_temp input columns are zero-padded on load).
"""
...
custom_imports = dict(
    imports=['my_code.custom_config.ours_ft_modules'],
...
feat_cache_name = 'feat_ours_pilot'
...
model = dict(
    ...
    head=dict(
        type='OursFtCascadedHead',
        in_channels=2048,
        out_channels=16,
        hmd_info_size=12,
        heatmap_decoder_type='efficient',
        decoder=codec,
        # feature-history branch (Ours-T shell; f_temp -> stage 2)
        history_mode='features',
        joint_proj_dim=32,
        f_temp_dim=64,
        history=10,
        temporal_embed=512,
...
# =============================================================================
data_mode = 'topdown'
dataset_type = 'KinectEgoposeFtDataset'
...
dataset_train = dict(
    ...
    feat_cache_name=feat_cache_name,
    history=10,
