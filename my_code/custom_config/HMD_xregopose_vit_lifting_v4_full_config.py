"""
ViT-Style Lifting Head v4 - CosineAnnealingLR (Full Training)

v3 대비 변경점:
- MultiStepLR [3, 5, 7] → CosineAnnealingLR (부드러운 LR 감소)
- 급격한 LR decay로 인한 validation spike 방지

Architecture: v3와 동일
- Learnable Joint Queries + Self-Attention (4 layers)
- Heatmap Reconstruction Regularization (loss_weight=500)
- HMD Cross-Attention
- Direct 3D Pose Head

LR Schedule:
    Epoch 0-10: Cosine decay from 0.0005 → 1e-6
    (vs v3: Step decay at epoch 3, 5, 7 with gamma=0.5)

Expected:
- 더 안정적인 validation 성능
- Walking 등 동적 action에서 스파이크 감소
"""
...
# =============================================================================
# LR Schedule - v4 KEY CHANGE: CosineAnnealingLR
# =============================================================================
param_scheduler = [
    # Warmup (iter-based)
    dict(
    ...
    # CosineAnnealingLR (smooth decay, no sudden drops)
    dict(
...
# =============================================================================
# Model - ViT-Style Lifting v4 (same architecture as v3)
# =============================================================================
model = dict(
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_vit_lifting_v4'),
...
work_dir = 'work_dirs/HMD_xregopose_vit_lifting_v4_full'
