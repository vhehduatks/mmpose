"""
ViT-Style Lifting Head v3 - With Heatmap Reconstruction (Full Training)

Option B: Heatmap Reconstruction from Joint Tokens
- Joint Tokens → Heatmap Decoder → Reconstructed Heatmap
- Heatmap Loss regularizes Joint Tokens (same latent path)
- Better 3D prediction through 2D info injection
...
Architecture:
    ...
    │         ↓                                                   │
    │  Joint Tokens [16, D]  ◄── Shared latent                   │
    │         │                                                   │
    │         ├──────────────────────┐                            │
    │         │                      ↓                            │
    │         │            ┌──────────────────┐                   │
    ...
    │         ↓                                                   │
    │  3D Loss (L2, cosine, limb, HMD)                           │
    └─────────────────────────────────────────────────────────────┘

Key: Heatmap loss forces 2D joint info INTO joint tokens,
     improving 3D prediction through shared representation.
...
# =============================================================================
# Model - ViT-Style Lifting v3 (WITH Heatmap Reconstruction)
# =============================================================================
model = dict(
    ...
    head=dict(
        ...
        heatmap_size=47,
        use_hmd=True,
        use_heatmap_recon=True,  # KEY: Enable reconstruction (Option B)
        # Losses
        loss_heatmap_recon=dict(  # Reconstruction loss (regularizes joint tokens)
            loss_weight=500,
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_vit_lifting_v3'),
...
work_dir = 'work_dirs/HMD_xregopose_vit_lifting_v3_full'
