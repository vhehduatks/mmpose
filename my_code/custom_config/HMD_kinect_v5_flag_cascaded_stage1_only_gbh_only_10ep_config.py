"""
egodataset_flag_fixed_ver5 - Cascaded Stage 1 Only + GBH-only (10 epochs)
- For paper TABLE V: Single-stage + GBH-only column
- Apples-to-apples with the 70.24mm Single-stage + LHF row:
  same head (CascadedRefinementHead_enhanced, use_refinement=False), same
  schedule, only the HMD input differs — LHF dims zeroed, GBH dims kept.
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='egodataset_flag_fixed_ver5-cascaded-stage1-only-gbh-only',
            name='HMD_kinect_v5_flag_cascaded_stage1_only_gbh_only_10ep',
            tags=['kinect_v5_flag', 'cascaded_stage1_only', 'gbh_only', '10ep'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_kinect_v5_flag_cascaded_stage1_only_gbh_only_10ep'
