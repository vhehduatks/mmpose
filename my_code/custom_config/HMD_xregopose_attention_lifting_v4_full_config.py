"""
Attention-based Lifting Head v4 - High LR + CosineAnnealing
- LR 0.001 (same as v3)
- CosineAnnealing (from v2, but NO warmup)
- Gradient clipping (max_norm=1.0)

Previous results:
- v1 (LR=0.0005, MultiStepLR): 45.43mm at epoch 7
- v2 (LR=0.0005, Warmup+Cosine): 48.38mm at epoch 8 (warmup slowed learning)
- v3 (LR=0.001, MultiStepLR): 46.68mm at epoch 1, then diverged

v4 Hypothesis:
- v3's high LR found good direction quickly (epoch 1: 46.68mm)
- v3 diverged because LR=0.001 was maintained for 4 epochs
- CosineAnnealing starts decaying immediately after epoch 1
- Expected: Fast initial convergence + stable decay = best of both worlds
"""
...
# =============================================================================
# Training Config - High LR + CosineAnnealing (NO warmup)
# =============================================================================
...
# Optimizer: LR=0.001 (same as v3) + Gradient Clipping
optim_wrapper = dict(
    optimizer=dict(lr=0.001, type='AdamW', weight_decay=0.01),
    clip_grad=dict(max_norm=1.0, norm_type=2),
)

# LR Schedule: CosineAnnealing WITHOUT warmup (starts decaying from epoch 1)
param_scheduler = [
    dict(
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_attention_lifting_v4'),
...
work_dir = 'work_dirs/HMD_xregopose_attention_lifting_v4_full'
