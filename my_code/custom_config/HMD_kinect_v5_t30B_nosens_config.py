"""Task 26 arm A — ego-cam-native stage 2 (dual-frame joint tokens +
rotation-invariant relational bias) + 6DoF dual-frame controller tokens,
END-TO-END retrain (backbone reopened, headline-lifter init). See
ours_ec_modules.py and HANDOFF.md Task 26 brief.

  stage2_frame='egocam', use_bias=True, use_sensor_ego=True, use_sensor_rot=True

Launch:
    PYTHONPATH=$PWD /home/.conda/envs/mmpose/bin/python tools/train.py \
        my_code/custom_config/HMD_kinect_v5_t26_ecA_config.py \
        --work-dir work_dirs/t30B_nosens
"""
...
custom_imports = dict(
    imports=['my_code.custom_config.ours_ec_modules'],
...
# headline lifter (backbone + encoder + pose_decoder + heatmap init); fusion
# is dropped and stage-2 is new, so this is a full end-to-end retrain
...
# ---- training (Task 24 full-retrain recipe: 10ep AdamW 5e-4 [5,8]) ----
...
custom_hooks = [dict(type='SyncBuffersHook')]   # end-to-end: NO freeze hook
...
model = dict(
    ...
    head=dict(
        type='OursEgoCamHead',
        stage2_frame='egocam', use_bias=True,
        use_sensor_ego=True, use_sensor_rot=True,
        use_sensor_tokens=False,
        in_channels=2048, out_channels=16, hmd_info_size=12,
        heatmap_decoder_type='efficient', decoder=codec,
...
work_dir = 'work_dirs/t30B_nosens'
