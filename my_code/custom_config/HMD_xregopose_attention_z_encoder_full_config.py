"""
Attention Z Encoder Head - Full Dataset Training

Key Idea:
    Replace Baseline's GAP with cross-attention spatial aggregation.
    Conv→CrossAttn(queries, spatial_tokens)→[B,256] (selective attention)
    Residual gate ensures training starts identical to baseline (gate init≈0).

    All downstream: PoseDecoder, HeatmapDecoder, HMD fusion unchanged.

Additional losses:
    - bone_length: Enforce per-limb bone length matching
    - symmetry: Left-right limb length symmetry
    - pose_l2norm_weighted: Higher weight on lower body (no HMD)
"""
...
# =============================================================================
# Model - Attention Z Encoder Head
# =============================================================================
model = dict(
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_attention_z_encoder'),
...
work_dir = 'work_dirs/HMD_xregopose_attention_z_encoder_full'
