"""
Enhanced HMD Info - Ground Reference (11 dims)

Rationale:
    In real HMD deployment, we can measure:
    - HMD height from ground (tracking system provides this)
    - Controller heights from ground

    The 'torso_reference' mode uses pelvis position, which is NOT directly
    measurable from HMD in practice.

    This mode estimates ground from feet positions (min of feet Y),
    then computes head height from ground - closer to real-world HMD data.

Changes from baseline (41.37mm):
    - Add EnhanceHMDInfo transform with mode='ground_reference'
    - Use CustomxRegoposeBaselinel1_enhanced_hmd head with hmd_info_size=11
    - HMD info: 9 base dims + 2 (head_from_ground, head_torso_dist) = 11 dims

Comparison with torso_reference:
    - torso_reference: [torso_y, head_torso_dist] - uses pelvis position
    - ground_reference: [head_from_ground, head_torso_dist] - uses estimated ground

Target: sub-41mm with more realistic HMD-deployable features
"""
...
model = dict(
    ...
    head=dict(
        ...
        in_channels=2048,
        out_channels=16,
        hmd_info_size=11,  # 9 base + 2 (head_from_ground, head_torso_dist)
        decoder=codec,
        loss=dict(
            loss_weight=1000,
...
train_pipeline = [
    ...
    dict(type='EnhanceHMDInfo', mode='ground_reference'),  # Ground-based height
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_enhanced_hmd_ground_ref'),
...
work_dir = 'work_dirs/HMD_xregopose_enhanced_hmd_ground_ref_full'
