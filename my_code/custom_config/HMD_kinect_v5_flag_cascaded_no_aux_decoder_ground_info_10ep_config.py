"""
egodataset_flag_fixed_ver5 - Cascaded Refinement V2b + Ground Info 12-dim, NO Auxiliary Decoders (10 epochs)
- Ablation: disables heatmap reconstruction decoder and HMD reconstruction loss
- Tests whether auxiliary decoders (latent space regularization) improve performance
...
- Head: CustomEgoposeCascadedRefinementHead_enhanced (use_auxiliary_decoders=False)
- Epochs: 10
"""
...
model = dict(
    ...
    head=dict(
        ...
        out_channels=16,
        hmd_info_size=12,
        use_auxiliary_decoders=False,  # Ablation: no heatmap recon + no HMD recon
        heatmap_decoder_type='efficient',
        decoder=codec,
        # Stage 1 losses (no auxiliary decoder losses)
        loss=dict(
            loss_weight=1000,
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='egodataset_flag_fixed_ver5-cascaded-no-aux-decoder-ground-info-12dim',
            name='HMD_kinect_v5_flag_cascaded_no_aux_decoder_ground_info_10ep',
            tags=['kinect_v5_flag', 'cascaded_no_aux_decoder', 'ground_info', '10ep', 'ablation'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_kinect_v5_flag_cascaded_no_aux_decoder_ground_info_10ep'
