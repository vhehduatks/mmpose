"""
Attention-based Lifting Head v3 - Higher LR (0.001)
- LR 0.001 (2x of v1)
- MultiStepLR [4, 7] (same as v1)
- No warmup
- Gradient clipping (max_norm=1.0)

Comparison:
- v1 (LR=0.0005, MultiStepLR): 45.43mm at epoch 7
- v2 (LR=0.0005, Warmup+Cosine): 48.38mm at epoch 8 (worse - warmup caused slow start)
- v3 (LR=0.001, MultiStepLR): ???

v2 실패 분석:
- Warmup이 초기 학습 지연 (epoch 1: 111mm vs v1: 55mm)
- CosineAnnealing이 Attention에 부적합
- Epoch 5에서 spike 발생 (57.78mm)

v3 가설: 높은 LR(0.001)로 빠른 수렴 + MultiStepLR 유지
"""
...
# =============================================================================
# Training Config - Higher LR (0.001)
# =============================================================================
...
# Optimizer: LR=0.001 (2x of v1) + Gradient Clipping
optim_wrapper = dict(
    optimizer=dict(lr=0.001, type='AdamW', weight_decay=0.01),
    clip_grad=dict(max_norm=1.0, norm_type=2),
)

# LR Schedule: MultiStepLR [4, 7] (same as v1)
param_scheduler = [
    dict(
        type='MultiStepLR',
        begin=0,
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_attention_lifting_v3'),
...
work_dir = 'work_dirs/HMD_xregopose_attention_lifting_v3_full'
