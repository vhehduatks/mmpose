"""
ViT Lifting v6 + Lower Body Enhancement (Full Training Config)

Base: ViT Lifting v6 (SPT + LSA + DWConv)
New losses for lower body improvement:
    1. pose_l2norm_weighted: Lower body joints (8-15) get 1.5x loss weight
    2. bone_length_loss: Enforce actual per-limb bone length matching
    3. symmetry_loss: Left-right limb length symmetry constraint

Expected improvements:
    - bone_length_loss: Prevents unrealistic limb proportions (2-4mm)
    - pose_l2norm_weighted: Forces model to attend more to lower body (1-2mm)
    - symmetry_loss: Enforces bilateral leg symmetry (1-2mm)

Architecture:
    Same as v6 (SPT + LSA + DWConv)
    + Enhanced loss supervision for lower body

Loss summary:
    - pose_l2norm_weighted (w=1.0, lower=1.5x): Weighted L2 norm
    - cosine_similarity (w=0.1): Limb direction
    - limb_length (w=0.25): L1 distance
    - bone_length_loss (w=0.5): Per-limb bone length constraint
    - symmetry_loss (w=0.1): Left-right symmetry
    - heatmap_recon (w=500): Heatmap reconstruction
    - hmd (w=1.0): HMD reconstruction
"""
...
# =============================================================================
# Model - ViT Lifting v6 + Lower Body Enhancement
# =============================================================================
model = dict(
    ...
    head=dict(
        ...
        # === Losses (enhanced for lower body) ===
        # [Changed] pose_l2norm -> pose_l2norm_weighted (lower body 1.5x)
        loss_pose_l2norm=dict(
            loss_weight=1.0,
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_vit_v6_lower_body'),
...
work_dir = 'work_dirs/HMD_xregopose_vit_lifting_v6_lower_body_full'
