"""
Cascaded Refinement + Both From Ground V2a (EfficientDecoder + Stronger MLP)

Optimizations over V1 (37.88mm):
  1. EfficientHeatmapDecoder: 40M -> 1.35M params
  2. Stronger Refinement MLP:
     - refinement_num_stage: 1 -> 2 (more depth)
     - refinement_dropout: 0.5 -> 0.3 (less regularization)

Hypothesis: V1's single-stage MLP with 0.5 dropout may be underfitting.
More capacity could improve refinement quality.

Target: Improve over V1 (37.88mm) with stronger refinement.
"""
...
# Model with EfficientHeatmapDecoder + Stronger Refinement MLP
model = dict(
    ...
    head=dict(
        ...
        # V2 Optimization 1: EfficientHeatmapDecoder
        heatmap_decoder_type='efficient',
        # V2a Optimization 2: Stronger Refinement MLP
        refinement_num_stage=2,   # V1: 1 -> V2a: 2
        refinement_dropout=0.3,   # V1: 0.5 -> V2a: 0.3
        decoder=codec,
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_cascaded_both_from_ground_v2a'),
...
work_dir = 'work_dirs/HMD_xregopose_cascaded_both_from_ground_v2a_full'
