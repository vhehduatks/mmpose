"""Task 11.1 — Ours-2Dt: inertially-aligned temporal 2D prior channels
(single-frame; per-joint [d_f (3), h (1)] injected at stage 2's feature
sampling point).

Fine-tunes the whole head (stage 1 + stage 2) from the headline checkpoint,
backbone frozen (FreezeBackboneHook). The prior channels come from the
FROZEN baseline stage 1's previous-frame 2D (kp2d caches; Val cache = the
frozen model's own Val outputs -> exact test-time semantics), warped by the
HMD infinite homography. Control config: warp_mode='identity'.
...
Launch (mmpose env; PYTHONPATH so custom_imports resolves):
    ...
    /home/.conda/envs/mmpose/bin/python tools/train.py \
        my_code/custom_config/HMD_kinect_v5_ours_2dt_config.py \
        --work-dir work_dirs/ours_2dt

GATE 11b: a val run of this config with the headline checkpoint and
`zero_prior=True` on the head must reproduce 64.357/60.909/67.805 exactly
...
custom_imports = dict(
    imports=['my_code.custom_config.ours_2dt_modules'],
...
model = dict(
    ...
    head=dict(
        type='OursPrior2DHead',
        in_channels=2048,
        out_channels=16,
        hmd_info_size=12,
        heatmap_decoder_type='efficient',
        decoder=codec,
        # warped-keypoint prior channels (16 ch -> stage-1 final conv)
        zero_prior=False,
...
# =============================================================================
data_mode = 'topdown'
dataset_type = 'KinectEgopose2DtDataset'
