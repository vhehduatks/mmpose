"""
Kinect v4 Cascaded Refinement + No-HMD 10-epoch (vision-only)
- Dataset: KinectEgoposeDataset (Azure Kinect 32->16 joint mapping)
- Backbone: ResNet-101 pretrained on COCO
- Head: CustomEgoposeCascadedRefinementHead_enhanced (hmd_info_size=9, zeros)
  - Stage 1: Deconv->Heatmap->EnhancedEncoder->Z->PoseDecoder->Coarse3D
  - Stage 2: GridSample + KinFeatures + HMD(zeros) -> RefinementMLP -> delta
...
# =============================================================================
# Model - Cascaded Refinement + No HMD (vision-only, zeros)
# =============================================================================
model = dict(
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='kinect-cascaded-no-hmd',
            name=f'kinect_v4_cascaded_no_hmd_{_max_epochs}ep',
            tags=['kinect', 'cascaded', 'no_hmd', 'vision_only', '10ep'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_kinect_v4_cascaded_no_hmd_10ep'
