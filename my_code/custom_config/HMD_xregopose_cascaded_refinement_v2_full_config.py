"""
Cascaded Pose Refinement Head V2 - Full Dataset Training

Improvements over V1 (42.86mm):
  1. Pretrained Stage 1 from Baseline checkpoint (load_from)
     - V1 wasted ~4 epochs converging Stage 1 (122.77mm at epoch 1)
     - Loading pretrained baseline (41.37mm) gives Stage 1 a strong start
  2. Auxiliary structural losses on Stage 1 coarse pose
     - bone_length_coarse (w=0.5) + symmetry_coarse (w=0.1)
     - Proven effective in Attention Z Encoder experiment
  3. Stronger refinement MLP
     - 2 stages (vs 1 in V1) with 0.3 dropout (vs 0.5)
     - More capacity for learning residual corrections

Target: sub-42mm (vs 42.86mm in V1)
"""
...
# Load pretrained Baseline checkpoint for Stage 1
# mmengine loads with strict=False: matching keys loaded, Stage 2 modules init randomly
...
model = dict(
    ...
    head=dict(
        type='CustomEgoposeCascadedRefinementHeadV2',
        in_channels=2048,
        out_channels=16,
        decoder=codec,
        ...
        loss_bone_length_coarse=dict(loss_weight=0.5, type='bone_length_loss'),
        loss_symmetry_coarse=dict(loss_weight=0.1, type='symmetry_loss'),
        ...
        # Refinement params (V2: stronger MLP)
        refinement_num_stage=2,
        refinement_dropout=0.3,
    ),
    test_cfg=dict(
        flip_test=False,
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_cascaded_refinement_v2'),
...
work_dir = 'work_dirs/HMD_xregopose_cascaded_refinement_v2_full'
