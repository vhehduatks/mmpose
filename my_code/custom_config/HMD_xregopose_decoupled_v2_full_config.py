"""
Upper-Lower Decoupled Head v2 - MultiStepLR (Full Training Config)

Change from Decoupled v1:
    - Removed LinearLR warmup (proven harmful for attention heads)
    - Replaced CosineAnnealingLR with MultiStepLR [4, 7] gamma=0.5
    - Kept weight_decay=0.01 and clip_grad (needed for complex head)

Rationale:
    Decoupled v1 had severe validation oscillation (+/-12mm) with
    CosineAnnealingLR + warmup. Attention Lifting v1-v7 experiments
    showed that:
    1. Warmup is counterproductive for attention-based heads
    2. MultiStepLR [4, 7] outperforms CosineAnnealing for attention
    3. Abrupt LR drop stabilizes attention weights faster

    v1 LR pattern: Warmup -> CosineAnnealing (slow decay, oscillation)
    v2 LR pattern: MultiStepLR [4,7] gamma=0.5 (fast decay at milestones)
        Epoch 1-4: LR = 5e-4  (full LR from the start)
        Epoch 5-7: LR = 2.5e-4 (after milestone 4)
        Epoch 8-10: LR = 1.25e-4 (after milestone 7)

Architecture (unchanged from Decoupled v1):
...
# =============================================================================
# Optimizer (keep weight_decay and clip_grad for complex head)
# =============================================================================
optim_wrapper = dict(
...
# =============================================================================
# LR Schedule - MultiStepLR [4, 7] (no warmup)
# =============================================================================
param_scheduler = [
    dict(
        type='MultiStepLR',
...
# =============================================================================
# Model - Upper-Lower Decoupled Head (unchanged)
# =============================================================================
model = dict(
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_decoupled_v2'),
...
work_dir = 'work_dirs/HMD_xregopose_decoupled_v2_full'
