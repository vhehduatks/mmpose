"""
egodataset_flag_fixed_ver5 - Cascaded Stage 1 Only + LHF+GBH (10 epochs)
- For paper TABLE V: Single-stage + LHF+GBH column (re-run, parallel naming with
  *_image_only / *_gbh_only / *_lhf_only siblings).
- Apples-to-apples sibling of HMD_kinect_v5_flag_cascaded_stage1_only_gbh_only_10ep_config.py:
  same head (CascadedRefinementHead_enhanced, use_refinement=False), same
  schedule, only the HMD input differs — both LHF and GBH dims kept.
- HMD input: 12-dim, dims 0-8 (LHF) + dims 9-11 (GBH ground heights) all active.
...
- Head: CustomEgoposeCascadedRefinementHead_enhanced (Stage 1 only, hmd_info_size=12)
- Epochs: 10
"""
...
model = dict(
    ...
    head=dict(
        ...
        in_channels=2048,
        out_channels=16,
        hmd_info_size=12,             # 9 LHF + 3 GBH ground heights (full HMD)
...
dataset_train = dict(
    ...
    ground_info_mode='both_from_ground',  # 12-dim: LHF + GBH ground heights
    use_hmd=True,                           # HMD active
    use_lhf=True,                           # Keep LHF dims (full HMD)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='egodataset_flag_fixed_ver5-cascaded-stage1-only-lhf-gbh',
            name='HMD_kinect_v5_flag_cascaded_stage1_only_lhf_gbh_10ep',
            tags=['kinect_v5_flag', 'cascaded_stage1_only', 'lhf_gbh', '10ep'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_kinect_v5_flag_cascaded_stage1_only_lhf_gbh_10ep'
