"""Task 41-B — world-consistency-calibrated canon on the E2' attention arm
(kinect). Head = the Task 39 E2' gate arm (OursAttnCascadedHead,
attn_mode='replace', pe_mode='fixed') with canon_mode='world_calib':
canon(t) = compute_relpose_to_floor(m2w(t)) @ T_learn, T_learn a
per-participant SE(3) initialized from the 41-A grid-probe argmax
(t41_probe_results.npy) and anchored by small verticality/floor aux losses.
Deployed path reads temporal_mid2world + hmd12 ONLY — temporal_cam2world is
never touched (T_ec_hmd ban). Dataset = the t26 SensorV3 assembly (validated
by the t37 exporter: Val through it reproduces 64.36 exactly).
Training regime matches the E2'/P2 gate configs: COCO-pretrained backbone,
NO load_from, 10ep AdamW 5e-4 [5,8], batch 16, Val-selected.

Launch (dist 2-GPU):
    bash tools/dist_train.sh \
        my_code/custom_config/HMD_kinect_v5_gbh_wcalib_config.py 2 \
        --work-dir work_dirs/t41_wcalib
"""
...
wcalib_init = '/mnt/linux_hdd_a/t41_probe_results.npz'
...
model = dict(
    ...
    head=dict(
        ...
        attn_mode='replace',
        use_canon=True,
        canon_mode='world_calib',
        wcalib_init=wcalib_init,
        wcalib_vert_w=0.05,
        wcalib_floor_w=0.05,
        pe_mode='fixed',
        sensor_mode='off',
