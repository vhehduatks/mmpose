"""
Single-stage architecture, vision-only (no HMD input).
use_refinement=False + ZeroHMDInfo.
Isolates pure architectural contribution without any HMD leakage.
"""
...
model = dict(
    ...
    head=dict(
        ...
        in_channels=2048,
        out_channels=16,
        hmd_info_size=9,  # Vision-only: zeroed HMD input
...
train_pipeline = [
    ...
    dict(type='ZeroHMDInfo', hmd_dim=9),
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_single_stage_no_hmd'),
...
work_dir = 'work_dirs/HMD_xregopose_single_stage_no_hmd'
