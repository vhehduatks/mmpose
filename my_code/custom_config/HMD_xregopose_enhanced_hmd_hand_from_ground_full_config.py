"""
Enhanced HMD Info - Hand From Ground (11 dims)

Rationale:
    Hand heights from estimated ground plane.
    In HMD systems, controller positions can be tracked relative to floor.

    This mode provides:
    - left_hand_from_ground: Left controller height from floor
    - right_hand_from_ground: Right controller height from floor

    Unlike head_from_ground, this focuses on hand positions which may help
    with upper body estimation or overall pose coherence.

Changes from baseline (41.37mm):
    - Add EnhanceHMDInfo transform with mode='hand_from_ground'
    - Use CustomxRegoposeBaselinel1_enhanced_hmd head with hmd_info_size=11
    - HMD info: 9 base dims + 2 (left_hand_from_ground, right_hand_from_ground) = 11 dims

Hypothesis:
    Hand heights from ground may provide different information than head height.
    The existing 9-dim HMD info already has hand positions relative to head,
    but absolute ground reference may help with depth estimation.

Target: Compare with head_from_ground and ground_reference modes.
"""
...
# Model with enhanced HMD head (11 dims)
model = dict(
...
train_pipeline = [
    ...
    dict(type='EnhanceHMDInfo', mode='hand_from_ground'),  # Hand heights from ground
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_enhanced_hmd_hand_from_ground'),
...
work_dir = 'work_dirs/HMD_xregopose_enhanced_hmd_hand_from_ground_full'
