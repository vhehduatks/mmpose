"""
Ground Info Visualization Smoke Test

Quick test config to verify the new CustomPose3dLocalVisualizer_xregopose_ground_info
visualizer works correctly with wandb logging.

Features:
- Uses V2 cache
- CustomPose3dLocalVisualizer_xregopose_ground_info visualizer
- both_from_ground mode
- Short training (2 epochs) on small dataset
- Frequent visualization interval (every 50 steps)
"""
...
if IS_WINDOWS:
    ...
    cache_file_train = '/mnt/dataset_vol/h5cache/train_small_v2.h5'
    cache_file_val = '/mnt/dataset_vol/h5cache/val_small_v2.h5'
...
default_hooks = dict(
    checkpoint=None,  # No checkpoint saving for smoke test
    visualization=dict(
        enable=True,
        interval=5,  # Visualize every 5 iterations (val has only 16 iters)
        kpt_thr=0.3,
...
model = dict(
    ...
    head=dict(
        ...
        in_channels=2048,
        out_channels=16,
        hmd_info_size=12,  # 9 base + 3 (both_from_ground)
...
if IS_WINDOWS:
    ...
    _batch_size = 32  # Small batch for smoke test
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_ground_info_vis_smoke_test'),
        type='WandbVisBackend'
    ),
]

# NEW: Use the ground info visualizer
visualizer = dict(
    name='visualizer',
...
work_dir = 'work_dirs/HMD_xregopose_ground_info_vis_smoke_test'
