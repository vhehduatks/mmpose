"""
XR EgoPose Training Config - Lifting + Backbone Feature Fusion (Phase 5-C)
...
Key Design:
- Role Separation:
  - Heatmap: 2D position only (gradient blocked from 3D loss)
  - Backbone: 3D depth cues (gradient flows from 3D loss)
- coords_2d.detach(): Prevents 3D loss from affecting heatmap learning
- Z_backbone: Backbone encoder provides depth information

Comparison:
- Phase 5-B (Lifting only): 190mm MPJPE - failed due to lack of depth info
- Phase 5-C (+ Backbone Fusion): Expected improvement with depth cues

Losses:
- loss_heatmap: MSE(pred_heatmap, gt_heatmap) - weight 1000 (2D)
- loss_coord: MSE(soft_argmax coords, gt_coords) - weight 10 (2D)
- loss_pose_l2norm: L2(pred_3d, gt_3d) - weight 1.0 (3D)
...
# =============================================================================
# Model Architecture - Lifting + Backbone Feature Fusion (Phase 5-C)
# =============================================================================
model = dict(
    ...
    head=dict(
        type='CustomEgoposeLiftingBackboneFusionHead',
        decoder=codec,
        in_channels=2048,
        out_channels=16,
        ...
        # Backbone encoder config (NEW: for depth cues)
        backbone_latent_dim=256,
...
train_dataloader = dict(
    batch_size=64,  # Larger batch possible (lighter model)
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_lifting_backbone_fusion'),
...
work_dir = 'work_dirs/HMD_xregopose_lifting_backbone_fusion'
