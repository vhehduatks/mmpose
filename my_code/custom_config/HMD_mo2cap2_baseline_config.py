"""
Mo2Cap2 Baseline Model - Simple Heatmap to 3D Lifting

Model: Simple heatmap-to-lifting without cascaded refinement
Architecture: Backbone -> Deconv -> Heatmap -> Encoder (with HMD) -> Pose Decoder -> 3D

This is for comparison with the cascaded refinement model.
Uses the same HMD info (12-dim) as the V2 model.
...
# Model: Baseline head with hmd_info_size=12 (same as V2)
model = dict(
    ...
    head=dict(
        ...
        in_channels=2048,
        out_channels=15,
        hmd_info_size=12,  # 9 base + 3 ground ref (same as cascaded V2)
...
# Pipeline with both_from_ground mode (same as V2)
train_pipeline = [
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            name='baseline_both_from_ground',
            tags=['mo2cap2', 'baseline', 'both_from_ground'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_mo2cap2_baseline'
