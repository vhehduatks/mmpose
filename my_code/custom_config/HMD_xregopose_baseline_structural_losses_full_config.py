"""
Baseline + Structural Losses - Full Dataset Training

Hypothesis:
    The improvement in Attention Z Encoder (43.79mm vs Baseline 41.37mm = +2.42mm)
    came from the additional structural losses, not the attention mechanism.
    The gate only reached 0.93% by epoch 10, meaning attention contributed <1%.

    Adding bone_length and symmetry losses directly to Baseline could:
    1. Match Attention Z Encoder's performance without attention overhead
    2. Potentially beat Baseline by enforcing physical constraints

Architecture: Identical to Baseline (CustomxRegoposeBaselinel1)
Changes: Only add structural losses (no architectural change)

Losses (8 total):
    - 6 from Baseline: loss_kpt, loss_pose_l2norm, loss_cosine_similarity,
                       loss_limb_length, loss_heatmap_recon, loss_hmd
    - 2 NEW: loss_bone_length (w=0.5), loss_symmetry (w=0.1)
"""
...
# =============================================================================
# Model - Baseline + Structural Losses
# =============================================================================
model = dict(
    ...
    head=dict(
        type='CustomxRegoposeBaselinel1',  # Same as Baseline
        in_channels=2048,
        out_channels=16,
        decoder=codec,
        # Baseline losses (unchanged)
        loss=dict(
            loss_weight=1000,
        ...
        # NEW: Structural losses (from Attention Z Encoder)
...
vis_backends = [
    ...
    dict(
        init_kwargs=dict(project='mmpose_xregopose_baseline_structural_losses'),
...
work_dir = 'work_dirs/HMD_xregopose_baseline_structural_losses_full'
