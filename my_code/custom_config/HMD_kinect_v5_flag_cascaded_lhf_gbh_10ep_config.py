"""
egodataset_flag_fixed_ver5 - Cascaded Refinement V2b + LHF+GBH (10 epochs)
- For paper TABLE V: Cascaded + LHF+GBH column (full 12-dim HMD)
- Apples-to-apples sibling of HMD_kinect_v5_flag_cascaded_gbh_only_10ep_config.py:
  same head (CascadedRefinementHead_enhanced, use_refinement default True),
  same schedule, only the HMD input differs — both LHF and GBH dims kept.
- HMD input: 12-dim, dims 0-8 (LHF) + dims 9-11 (GBH ground heights)
  (use_lhf default True, GBH ground heights enabled via ground_info_mode)
...
- Head: CustomEgoposeCascadedRefinementHead_enhanced (hmd_info_size=12)
- Epochs: 10
"""
...
dataset_train = dict(
    ...
    use_lhf=True,                           # Full HMD: keep LHF dims (default)
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            project='egodataset_flag_fixed_ver5-cascaded-lhf-gbh',
            name='HMD_kinect_v5_flag_cascaded_lhf_gbh_10ep',
            tags=['kinect_v5_flag', 'cascaded', 'lhf_gbh', '10ep'],
        ),
    ),
]
...
work_dir = 'work_dirs/HMD_kinect_v5_flag_cascaded_lhf_gbh_10ep'
