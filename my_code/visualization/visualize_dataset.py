...
def parse_args():
    ...
    parser.add_argument('--config', type=str,
                        default='my_code/custom_config/HMD_xregopose_single_coco_full_config.py',
...
def draw_3d_skeleton(ax, keypoints, title='3D Keypoints', show_labels=False):
    """Draw 3D skeleton.
    ...
    if show_labels:
        for i, kpt in enumerate(keypoints):
            if i < len(EGOPOSE_KPT_NAMES):
                ax.text(kpt[0], kpt[1], kpt[2], f' {i}:{EGOPOSE_KPT_NAMES[i]}',
...
def save_3d_skeleton_views(keypoints, output_path, title_prefix=''):
    """Save 3D skeleton with multiple views as a separate enlarged image.

    HMD-tracked joints (Head, LeftHand, RightHand) are highlighted with:
    - Larger marker size (200 vs 100)
    - Star marker shape
    - Red edge color
    - [HMD] suffix in label
    ...
    # HMD-tracked joint indices (Head=1, LeftHand=4, RightHand=7)
    # Note: Using config.py order where Spine2=0, Head=1
    HMD_JOINT_INDICES = {1, 4, 7}  # Head, LeftHand, RightHand

    # Create figure with 2x2 layout for different views
    fig = plt.figure(figsize=(16, 16))

    # Define views: (elevation, azimuth, title)
    views = [
        (15, 70, 'Perspective View'),
        (0, 0, 'Front View (looking at +X)'),
        (0, 90, 'Side View (looking at +Y)'),
        (90, 0, 'Top View (looking down -Z)'),
    ]
    ...
    for idx, (elev, azim, view_title) in enumerate(views):
        ...
        # Draw non-HMD keypoints first (regular style)
        non_hmd_indices = [i for i in range(len(keypoints)) if i not in HMD_JOINT_INDICES]
        if non_hmd_indices:
            non_hmd_kpts = keypoints[non_hmd_indices]
            non_hmd_colors = [EGOPOSE_KPT_COLORS[i] for i in non_hmd_indices]
            ax.scatter(non_hmd_kpts[:, 0], non_hmd_kpts[:, 1], non_hmd_kpts[:, 2],
                       c=non_hmd_colors, s=100, marker='o', edgecolors='white', linewidths=1)

        # Draw HMD keypoints with highlight (larger, star marker, red edge)
        hmd_indices = [i for i in HMD_JOINT_INDICES if i < len(keypoints)]
        if hmd_indices:
            hmd_kpts = keypoints[hmd_indices]
            hmd_colors = [EGOPOSE_KPT_COLORS[i] for i in hmd_indices]
            ax.scatter(hmd_kpts[:, 0], hmd_kpts[:, 1], hmd_kpts[:, 2],
                       c=hmd_colors, s=250, marker='*', edgecolors='red', linewidths=2,
                       label='HMD Tracked')

        # Draw skeleton
        for link_idx, (i, j) in enumerate(EGOPOSE_SKELETON):
            if i < len(keypoints) and j < len(keypoints):
                pts = keypoints[[i, j]]
                ax.plot(pts[:, 0], pts[:, 1], pts[:, 2],
                        color=EGOPOSE_LINK_COLORS[link_idx], linewidth=3)

        # Draw joint labels (add [HMD] suffix for HMD-tracked joints)
        for i, kpt in enumerate(keypoints):
            if i < len(EGOPOSE_KPT_NAMES):
                if i in HMD_JOINT_INDICES:
                    label = f' {i}:{EGOPOSE_KPT_NAMES[i]} [HMD]'
                    color = 'red'
                else:
                    label = f' {i}:{EGOPOSE_KPT_NAMES[i]}'
                    color = 'black'
        ...
        ax.set_title(f'{title_prefix}{view_title}', fontsize=14)
    ...
    # Add legend note as figure text
    fig.text(0.5, 0.02, 'HMD Tracked Joints: Head (1), LeftHand (4), RightHand (7) - marked with ★ and red edge',
             ha='center', fontsize=12, style='italic',
             bbox=dict(boxstyle='round', facecolor='lightyellow', edgecolor='orange'))

    plt.tight_layout(rect=[0, 0.04, 1, 1])  # Leave space for legend text
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    print(f"Saved 3D skeleton: {output_path}")
    plt.close()
...
def visualize_sample(sample, output_path=None, output_3d_path=None, show=False, debug=False):
    """Visualize a single dataset sample.

    Args:
        sample: Dataset sample dictionary
        output_path: Path to save main visualization
        output_3d_path: Path to save separate 3D skeleton visualization
    ...
    # 5 rows: Row 0 = header, Rows 1-4 = heatmaps (3 + 4 + 4 + 4 = 15 heatmaps)
    ...
    if gt_heatmaps is not None:
        ...
        # Individual heatmaps - show first 15 in remaining space
        # Row 1: 3 heatmaps (columns 1, 2, 3) - indices 0, 1, 2
        # Row 2: 4 heatmaps (columns 0, 1, 2, 3) - indices 3, 4, 5, 6
        # Row 3: 4 heatmaps (columns 0, 1, 2, 3) - indices 7, 8, 9, 10
        # Row 4: 4 heatmaps (columns 0, 1, 2, 3) - indices 11, 12, 13, 14
        num_show = min(15, gt_heatmaps.shape[0])
        for i in range(num_show):
            if i < 3:
                # Row 1: columns 1, 2, 3 (column 0 is sum heatmap)
                row = 1
                col = i + 1
            else:
                # Row 2+: 4 per row
    ...
    # Save separate 3D skeleton visualization with multiple views and labels
    if output_3d_path and gt_keypoints_3d is not None:
        save_3d_skeleton_views(gt_keypoints_3d, output_3d_path)
...
def main():
    ...
    for i, idx in enumerate(indices):
        ...
        output_3d_path = os.path.join(args.output, f'{args.split}_sample_{idx:05d}_3d_skeleton.png')
        # Only print debug for first sample
        visualize_sample(sample, output_path, output_3d_path, args.show, debug=(args.debug and i == 0))
