...
class CustomPose3dLocalVisualizer_xregopose(PoseLocalVisualizer):
	...
	def _draw_3d_data_samples(self,
		...
		def _draw_3d_instances_kpts(keypoints,
			...
			for idx, (kpts, score, score_2d) in enumerate(
				...
				# ax.set_aspect('auto')
				# ax.auto_scale_xyz([x_3d], [y_3d], [z_3d])
		...
		if draw_gt and 'gt_instances' in pose_samples:
			...
			if 'lifting_target' in gt_instances:
				...
				if 'keypoints_visible' in gt_instances:
					...
					keypoints_visible = np.ones(keypoints.shape[:-1])
			elif 'keypoint3d' in gt_instances:
				keypoints = gt_instances.get('keypoint3d', gt_instances.keypoint3d)
			...
			_draw_3d_instances_kpts(gt_kpt, scores, scores_2d,
		...
		if 'keypoint_3d' in pred_instances:
			...
			if 'keypoints_visible' in pred_instances:
				...
				keypoints_visible = np.ones(keypoints.shape[:-1])
			pred_kpt = keypoints # (1, 16, 3)
			_draw_3d_instances_kpts(pred_kpt, scores, scores_2d,
