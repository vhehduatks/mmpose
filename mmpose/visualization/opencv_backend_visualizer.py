...
class OpencvBackendVisualizer(Visualizer):
	...
	def show(self,
		...
		if self.backend == 'matplotlib':
			...
			if not hasattr(self, '_cv_win_name'):
				self._cv_win_name = win_name
				cv2.namedWindow(f'{id(self)}', cv2.WINDOW_NORMAL)  # WINDOW_NORMAL 플래그 추가
				cv2.setWindowTitle(f'{id(self)}', win_name)
			else:
				cv2.setWindowTitle(f'{id(self)}', win_name)
			shown_img = self.get_image() if drawn_img is None else drawn_img
			...
			# 초기 창 크기 설정 (선택사항)
			height, width = shown_img.shape[:2]
			cv2.resizeWindow(str(id(self)), width, height)
