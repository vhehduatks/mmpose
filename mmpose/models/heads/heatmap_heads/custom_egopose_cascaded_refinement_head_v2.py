...
"""
Cascaded Pose Refinement Head V2 with Kinematic Prior

Improvements over V1 (42.86mm):
  1. Pretrained Stage 1 from Baseline checkpoint (load_from)
  2. Auxiliary structural losses on Stage 1 coarse pose (bone_length + symmetry)
  3. Increased refinement MLP capacity (2 stages, 0.3 dropout)
  4. Optional freeze_stage1 for future experiments

Two-stage architecture:
  Stage 1: Standard Baseline (Deconv -> Heatmap -> Encoder(GAP+HMD) -> Z -> PoseDecoder -> Coarse 3D)
  Stage 2: Kinematic-Aware Refinement (Grid Sample + Kinematic Features + Shared MLP -> delta_pose)
...
Stage 2 components:
  ...
  4. Shared RefinementMLP with BatchNorm + Dropout + residual -> delta_pose [16, 3]
  5. Refined Pose = Coarse Pose + delta_pose
"""
...
def compute_kinematic_features(pose_3d):
    ...
    Returns:
        kin_feat: [B, 60] (15 bones x (3 direction + 1 length))
    """
    kin_feats = []
...
class CustomEgoposeCascadedRefinementHeadV2(BaseHead):
    """Cascaded Pose Refinement Head V2.

    Improvements over V1:
      1. Supports loading pretrained Stage 1 from Baseline checkpoint (load_from)
      2. Optional auxiliary structural losses on Stage 1 coarse pose
      3. Stronger refinement MLP (2 stages, 0.3 dropout by default)
      4. Optional freeze_stage1 for ablation

    Stage 1: Standard Baseline (Deconv->Heatmap->Encoder->Z->PoseDecoder->Coarse3D)
    Stage 2: Kinematic-Aware Refinement (GridSample + KinFeatures + MLP -> delta_pose)
    """
    ...
    def __init__(self,
                 ...
                 # Stage 1 auxiliary structural losses (NEW in V2)
                 loss_bone_length_coarse: OptConfigType = None,
                 loss_symmetry_coarse: OptConfigType = None,
                 # Stage 2 losses
                 ...
                 # Refinement params (V2 defaults: stronger MLP)
                 refinement_hidden_size: int = 256,
                 refinement_num_stage: int = 2,
                 refinement_dropout: float = 0.3,
                 spatial_feat_dim: int = 64,
                 pose_feat_dim: int = 128,
                 kin_feat_dim: int = 64,
                 # Freeze Stage 1 (NEW in V2)
        ...
        # Stage 1 auxiliary structural losses (NEW in V2)
        if loss_bone_length_coarse is not None:
            self.loss_bone_length_coarse_module = MODELS.build(
                loss_bone_length_coarse)
        else:
            self.loss_bone_length_coarse_module = None

        if loss_symmetry_coarse is not None:
            self.loss_symmetry_coarse_module = MODELS.build(
                loss_symmetry_coarse)
        else:
            self.loss_symmetry_coarse_module = None
        ...
        # Freeze Stage 1 if requested (NEW in V2)
    ...
    def _freeze_stage1_modules(self):
        """Freeze all Stage 1 modules so only Stage 2 trains."""
        stage1_modules = [
            self.deconv_layers, self.add_deconv_layers,
            self.conv_layers, self.final_layer,
            self.encoder, self.heatmap_decoder,
            self.pose_decoder, self.hmd_linear,
        ]
    ...
    def loss(self, feats, batch_data_samples, train_cfg={}):
        ...
        # Stage 1 auxiliary structural losses (NEW in V2)
        if self.loss_bone_length_coarse_module is not None:
            loss_bone_coarse = self.loss_bone_length_coarse_module(
                coarse_pose, gt_keypoint_3d)
            losses.update(
                loss_bone_length_coarse=torch.mean(loss_bone_coarse))

        if self.loss_symmetry_coarse_module is not None:
            loss_sym_coarse = self.loss_symmetry_coarse_module(coarse_pose)
            losses.update(
                loss_symmetry_coarse=torch.mean(loss_sym_coarse))
