...
"""
Cascaded Pose Refinement Head with Enhanced HMD Info

Enhanced version of CascadedRefinementHead that:
1. Accepts configurable hmd_info_size (10, 11, or 12 instead of 9)
2. Adds enhanced HMD info to Stage 2 refinement MLP input

Stage 2 refinement now has global height context from enhanced HMD info,
which should help lower body prediction where depth ambiguity is common.

Stage 2 input changes:
  Before: coarse_xyz(3) + spatial(64) + Z(64) + pose_ctx(128) + kin(64) = 323
  After:  coarse_xyz(3) + spatial(64) + Z(64) + pose_ctx(128) + kin(64) + hmd(10-12) = 333-335
"""
...
def preprocess_hmd_data_batch(p3d):
    """Compute HMD reconstruction from predicted 3D pose (base 9 dims only)."""
...
class CustomEgoposeCascadedRefinementHead_enhanced(BaseHead):
    """Cascaded Pose Refinement Head with Enhanced HMD Info.

    Stage 1: Deconv→Heatmap→Encoder→Z→PoseDecoder→Coarse3D (with enhanced HMD)
    Stage 2: GridSample + KinFeatures + HMD → RefinementMLP → Δpose
    ...
    def refine(self, coarse_pose, heatmap, backbone_feat, z_latent, hmd_info=None):
        ...
        Args:
            coarse_pose: [B, 16, 3] coarse 3D prediction
        ...
        # 4. HMD encoding for Stage 2 (NEW)
    ...
    def decode(self, batch_outputs, batch_data_samples, backbone_feat=None):
        ...
        if self.use_refinement and backbone_feat is not None:
            ...
            refined_pose = self.refine(
                ...
                hmd_info=HMD_info  # Pass HMD info to Stage 2
            )
