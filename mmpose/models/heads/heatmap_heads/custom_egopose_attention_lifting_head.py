...
"""
Attention-based Lifting Head with EfficientHeatmapDecoder
...
Key Features:
- EfficientHeatmapDecoder: Z regularization via reconstruction loss (1.35M vs 40M)
- Backbone Cross-Attention: Selective depth querying per joint
- HMD Cross-Attention: 3D reference from head/hands
- Joint Self-Attention: Structural relationships between joints
"""
...
class BackboneCrossAttention(nn.Module):
    ...
    Q: Joint tokens [B, 16, D] - "이 2D 위치의 depth는?"
    K/V: Backbone tokens [B, 64, D] - "spatial depth 정보"
    """
    ...
    def forward(
        ...
        Returns:
            ...
            attn_weights: [B, 16, 64] attention map (visualizable)
        """
        ...
        # Cross attention: joints query backbone for depth
        attn_out, attn_weights = self.cross_attn(
            query=joint_tokens,  # [B, 16, D]
            key=backbone_kv,     # [B, 64, D]
            value=backbone_kv
        )
...
class AttentionLiftingNetwork(nn.Module):
    """
    Full attention-based lifting network.
    ...
    def forward(
        ...
        attn_info = {
            'backbone_attn': backbone_attn,  # [B, 16, 64] - which spatial location
...
class CustomEgoposeAttentionLiftingHead(BaseHead):
    """
    Attention-based Lifting Head with EfficientHeatmapDecoder.

    Combines:
    - EfficientHeatmapDecoder: Z regularization (1.35M params)
    - Backbone Cross-Attention: Selective depth querying
    - HMD Cross-Attention: 3D reference from head/hands
    - Joint Self-Attention: Structural relationships
    ...
    def __init__(
        ...
        self.lifting_network = AttentionLiftingNetwork(
    ...
    def forward_lifting(
        ...
        """
        Forward pass for attention-based lifting.
