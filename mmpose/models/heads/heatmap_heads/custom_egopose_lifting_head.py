...
"""
Custom EgoPose Lifting Head

Architecture:
    Backbone → Heatmap [16, 47, 47] (trained with Gaussian MSE)
             ↓
...
Advantages:
- Heatmap: Dense supervision (Gaussian MSE) for stable training
- Soft-argmax: Sub-pixel accuracy, explicit 2D representation
- Lifting: Proven approach (Martinez et al. 2017)
- No HeatmapDecoder needed (saves 40M params)
"""
...
class LiftingNetwork(nn.Module):
    """
    2D→3D Lifting Network (Martinez et al. 2017 style)

    Input: 2D coords [B, 16, 2] + confidence [B, 16] + HMD [B, 9]
    ...
    def __init__(self,
        ...
        # Input: 2D coords (16*2=32) + confidence (16) + HMD (9) = 57
        input_dim = num_joints * 2 + num_joints + hmd_dim
    ...
    def forward(self, coords_2d: Tensor, confidence: Tensor, hmd_info: Tensor) -> Tensor:
        """
        Args:
...
class CustomEgoposeLiftingHead(BaseHead):
    """
    EgoPose Head with Soft-argmax 2D→3D Lifting

    Pipeline:
        1. Backbone features → Deconv → Heatmap [16, 47, 47]
        2. Heatmap trained with Gaussian MSE loss (dense supervision)
        3. soft_argmax extracts 2D coords + confidence
        4. Lifting network: 2D + conf + HMD → 3D pose

    Benefits:
        - Dense supervision from heatmap loss (stable training)
        - Sub-pixel accuracy from soft-argmax
        - Explicit 2D representation (interpretable)
        - No HeatmapDecoder (saves ~40M params)
    """
    ...
    def forward(self, feats: Tuple[Tensor]) -> Tensor:
        """
        Forward pass: backbone features → heatmap
        ...
        Returns:
            heatmap: [B, K, 47, 47]
        """
        x = feats[-1]  # Use last feature map
    ...
    def forward_lifting(self, heatmaps: Tensor, hmd_info: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        """
        Soft-argmax + Lifting network
        ...
        # Lifting: 2D + conf + HMD → 3D
        pose_3d = self.lifting_network(coords_2d, confidence, hmd_info)
    ...
    def loss(self,
        ...
        Losses:
            1. loss_heatmap: MSE(pred_heatmap, gt_heatmap) - dense supervision
            2. loss_coord: MSE(soft_argmax coords, gt_coords) - sub-pixel accuracy
            3. loss_pose_l2norm: L2(pred_3d, gt_3d)
            4. loss_cosine_similarity: cosine sim loss
            5. loss_limb_length: limb length consistency
        """
        # Forward: backbone → heatmap
        ...
        pose_3d, coords_2d, confidence = self.forward_lifting(pred_heatmaps, hmd_info)

        # === Losses ===
        losses = dict()

        # 1. Heatmap MSE loss (dense supervision)
        loss_heatmap = self.loss_heatmap(pred_heatmaps, gt_heatmaps, keypoint_weights)
        losses['loss_heatmap'] = loss_heatmap

        # 2. Coordinate loss (sub-pixel accuracy)
        # Weight by keypoint visibility
    ...
    def predict(self,
        ...
        # Lifting: heatmap → 2D → 3D
        pose_3d, coords_2d, confidence = self.forward_lifting(batch_heatmaps, hmd_info)
