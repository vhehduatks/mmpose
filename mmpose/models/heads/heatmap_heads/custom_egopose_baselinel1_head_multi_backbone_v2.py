...
"""
Dual Backbone Head with Progressive Warmup for Mutual Learning

v2 Changes:
- Added progressive warmup for mutual learning loss
- current_epoch tracked by MutualLearningWarmupHook
- mutual_warmup_epochs: epochs with no mutual learning (preserve pretrained)
- mutual_rampup_epochs: epochs to linearly increase mutual learning weight
"""
...
class CustomxRegoposeBaselinel1_multi_backbone_v2(BaseHead):
	"""Dual Backbone Head with Progressive Warmup for Mutual Learning.

	v2 Changes from original:
	- Progressive warmup: gradually introduce mutual learning loss
	- Preserves pretrained knowledge in early epochs
	- Hook-based epoch tracking for warmup schedule

	Args:
		mutual_warmup_epochs (int): Epochs with no mutual learning. Default: 5
		mutual_rampup_epochs (int): Epochs to linearly increase weight. Default: 10
	"""
	...
	def __init__(self,
				...
				# Progressive warmup for mutual learning
				mutual_warmup_epochs: int = 5,
				mutual_rampup_epochs: int = 10):
		...
		# Progressive warmup for mutual learning
		self.mutual_warmup_epochs = mutual_warmup_epochs
		self.mutual_rampup_epochs = mutual_rampup_epochs
		self.current_epoch = 0  # Updated by MutualLearningWarmupHook
	...
	def get_mutual_loss_weight(self) -> float:
		"""Get progressive warmup weight for mutual learning loss.

		Returns:
			float: Weight for mutual learning loss (0.0 to 1.0)
			- epoch < warmup: 0.0 (no mutual learning, preserve pretrained)
			- warmup <= epoch < warmup+rampup: linear increase 0->1
			- epoch >= warmup+rampup: 1.0 (full mutual learning)
		"""
		if self.current_epoch < self.mutual_warmup_epochs:
			return 0.0
		elif self.current_epoch < self.mutual_warmup_epochs + self.mutual_rampup_epochs:
			progress = (self.current_epoch - self.mutual_warmup_epochs) / self.mutual_rampup_epochs
			return progress
		else:
			return 1.0
	...
	def loss(self,
		...
		Includes progressive warmup for mutual learning loss.
		"""
		...
		# Knowledge distillation losses with progressive warmup
		loss_backbone_latant_raw = self.loss_backbone_latant_module(backbone_feat, backbone_feat2)
		loss_backbone_heatmap = self.loss_backbone_heatmap_module(pred_fields_sub, gt_heatmaps, keypoint_weights)

		# Apply progressive warmup to mutual learning loss
		mutual_weight = self.get_mutual_loss_weight()
		loss_backbone_latant = loss_backbone_latant_raw * mutual_weight
		...
		# Log mutual learning weight for monitoring
		losses.update(mutual_weight=torch.tensor(mutual_weight, device=backbone_feat.device))
