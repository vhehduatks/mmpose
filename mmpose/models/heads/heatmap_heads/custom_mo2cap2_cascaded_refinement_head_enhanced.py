...
"""
Cascaded Pose Refinement Head with Enhanced HMD Info for Mo2Cap2 Dataset

Mo2Cap2 adaptation of CascadedRefinementHead_enhanced:
1. 15 output joints (Mo2Cap2 skeleton)
2. Mo2Cap2 skeleton connections
3. Mo2Cap2 joint indices for HMD computation
...
# Mo2Cap2 skeleton: (parent_idx, child_idx) pairs for 14 bones
MO2CAP2_SKELETON = [
    (0, 1),   # Neck -> RightArm
...
def soft_argmax_2d(heatmaps, temperature=1.0):
    ...
    confidence = hm_flat.max(dim=-1)[0]
    return coords, confidence


def compute_kinematic_features_mo2cap2(pose_3d):
    """Compute bone direction vectors and lengths from 3D pose for Mo2Cap2."""
    kin_feats = []
    for parent, child in MO2CAP2_SKELETON:
        bone_vec = pose_3d[:, child] - pose_3d[:, parent]
        bone_len = torch.norm(bone_vec, dim=-1, keepdim=True)
        kin_feats.append(torch.cat([bone_vec, bone_len], dim=-1))
    return torch.cat(kin_feats, dim=-1)


def preprocess_hmd_data_batch_mo2cap2(p3d):
    """Compute HMD reconstruction from predicted 3D pose for Mo2Cap2 (base 9 dims only).
    ...
    # Mo2Cap2 uses Neck as head equivalent
    head = p3d[:, 0]        # Neck
...
# =========================================================================
# Stage 1 components (with configurable hmd_info_size)
# =========================================================================
...
class LinearModel(nn.Module):
    def __init__(self, input_size=20, num_classes=15,
...
# =========================================================================
# Stage 2 components (with HMD info support)
# =========================================================================

class RefinementBlock(nn.Module):
    """Residual block with BatchNorm + ReLU + Dropout."""
...
class RefinementMLP(nn.Module):
    """Shared per-joint refinement network with optional HMD info."""
...
class CustomMo2Cap2CascadedRefinementHead_enhanced(BaseHead):
    """Cascaded Pose Refinement Head with Enhanced HMD Info for Mo2Cap2.

    Stage 1: Deconv->Heatmap->Encoder->Z->PoseDecoder->Coarse3D (with enhanced HMD)
    Stage 2: GridSample + KinFeatures + HMD -> RefinementMLP -> delta_pose

    Args:
        hmd_info_size (int): Size of HMD info input. Default: 9.
            Options: 10 (head_from_ground), 11 (hand_from_ground), 12 (both_from_ground)
    ...
    def __init__(self,
                 ...
                 out_channels: int = 15,  # Mo2Cap2 has 15 joints
        ...
        self.pose_decoder = LinearModel(
            input_size=64,
            num_classes=out_channels,  # 15 joints for Mo2Cap2
            linear_size=512,
            num_stage=1,
            p_dropout=0.3
        )
    ...
    def refine(self, coarse_pose, heatmap, backbone_feat, z_latent, hmd_info=None):
        """Stage 2: Refine coarse pose with HMD info support.

        Args:
            coarse_pose: [B, 15, 3] coarse 3D prediction
            heatmap: [B, 15, 47, 47] predicted heatmaps
            backbone_feat: [B, C, H, W] backbone features
            z_latent: [B, 64] latent vector
            hmd_info: [B, hmd_info_size] enhanced HMD info (optional)
        ...
        # 3. Kinematic chain features (Mo2Cap2 skeleton)
        kin_raw = compute_kinematic_features_mo2cap2(coarse_pose)
        kin_feat = self.kin_encoder(kin_raw)

        # 4. HMD encoding for Stage 2
    ...
    def decode(self, batch_outputs, batch_data_samples, backbone_feat=None):
        ...
        if backbone_feat is not None:
            coarse_pose = batch_3d_keypoints.reshape(-1, self.out_channels, 3)
    ...
    def loss(self, feats, batch_data_samples, train_cfg={}):
        ...
        hmd_recon = preprocess_hmd_data_batch_mo2cap2(coarse_pose)

        coarse_pose = coarse_pose.reshape(-1, self.out_channels, 3)
