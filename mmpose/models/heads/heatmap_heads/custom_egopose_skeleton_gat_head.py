...
"""
Skeleton Graph Attention Network (GAT) Lifting Head

Key difference from Attention Lifting:
- Replaces fully-connected Self-Attention with Skeleton-based Graph Attention
- Joints only attend to anatomically connected joints (based on skeleton structure)
- Explicit structural prior for better 3D pose estimation
      ...
      Joint embed [16, D]                             │
           ...
           └──────── Cross-Attention ─────────────────┘
                     ...
                     Depth-aware joints [16, D]
                              ...
                              ▼
                    ┌─────────────────────┐
                    │ Skeleton GAT × N    │  ← NEW: Graph Attention
                    │ (only connected     │
                    │  joints attend)     │
                    └─────────────────────┘
                              │
...
Skeleton Structure (16 joints, EgoPose):
    [0] Head ─── [1] Neck
                   │
         ┌────────┴────────┐
         │                 │
    [2] L_Shoulder    [5] R_Shoulder
         │                 │
    [3] L_Elbow       [6] R_Elbow
         │                 │
    [4] L_Wrist       [7] R_Wrist
...
# Default skeleton edges for 16-joint EgoPose model
# Format: (joint_i, joint_j) - bidirectional edges
DEFAULT_SKELETON_EDGES = [
    ...
    # Left leg
    (1, 8),    # neck - left_hip (through spine)
    (8, 9),    # left_hip - left_knee
    (9, 10),   # left_knee - left_ankle
    # Right leg
    (1, 11),   # neck - right_hip (through spine)
    (11, 12),  # right_hip - right_knee
    (12, 13),  # right_knee - right_ankle
    # Cross-body connections (symmetry)
    (2, 5),    # left_shoulder - right_shoulder
    (8, 11),   # left_hip - right_hip
]

# Additional second-order connections (2-hop neighbors for richer structure)
SECOND_ORDER_EDGES = [
    # Head to shoulders
    (0, 2), (0, 5),
    # Shoulders to elbows (cross)
    (2, 6), (5, 3),
    # Shoulders to hips
    (2, 8), (5, 11),
    # Hips to knees (cross)
...
class HeatmapEncoder(nn.Module):
    """Encode heatmap to latent vector Z."""
...
class BackboneCrossAttention(nn.Module):
    """Cross-attention: Joint tokens query backbone spatial features for depth."""

    def __init__(
        self,
        joint_dim: int = 64,
        backbone_channels: int = 2048,
        backbone_spatial: int = 64,
        num_heads: int = 4,
        dropout: float = 0.1
    ):
        super().__init__()
    ...
    def forward(
        ...
        backbone_tokens = self.backbone_proj(backbone_feat)
        backbone_tokens = backbone_tokens.flatten(2).transpose(1, 2)
        backbone_kv = self.backbone_to_kv(backbone_tokens)
...
class SkeletonGraphAttention(nn.Module):
    """
    Graph Attention layer that respects skeleton structure.

    Instead of full self-attention (all joints attend to all joints),
    joints only attend to anatomically connected neighbors.

    Key features:
    - Skeleton-based attention mask
    - Optional second-order connections (2-hop neighbors)
    - Multi-head attention with edge-aware computation

    Args:
        joint_dim: Dimension of joint features
        num_joints: Number of joints (default: 16)
        num_heads: Number of attention heads
        dropout: Dropout rate
        skeleton_edges: List of (i, j) tuples defining connections
        use_second_order: Whether to include 2-hop neighbors
    """
    ...
    def __init__(
        ...
        self.head_dim = joint_dim // num_heads

        assert joint_dim % num_heads == 0, "joint_dim must be divisible by num_heads"
        ...
        # Q, K, V projections
        self.q_proj = nn.Linear(joint_dim, joint_dim)
        self.k_proj = nn.Linear(joint_dim, joint_dim)
        self.v_proj = nn.Linear(joint_dim, joint_dim)

        # Output projection
        self.out_proj = nn.Linear(joint_dim, joint_dim)

        # Edge-type embedding (optional: different attention for different edge types)
        # For now, we use a simpler approach with just the mask
        ...
        # Learnable edge attention bias (optional enhancement)
        self.edge_bias = nn.Parameter(torch.zeros(num_heads, num_joints, num_joints))
    ...
    def _build_adjacency_matrix(
        ...
        Returns:
            adj_matrix: [num_joints, num_joints] boolean tensor
                        True = can attend, False = cannot attend
        """
        adj = torch.zeros(num_joints, num_joints, dtype=torch.bool)

        # Self-loops (each joint attends to itself)
        for i in range(num_joints):
            adj[i, i] = True

        # First-order edges (direct connections)
        for i, j in edges:
            if i < num_joints and j < num_joints:
                adj[i, j] = True
                adj[j, i] = True  # Bidirectional

        # Second-order edges (optional)
        if use_second_order:
            for i, j in SECOND_ORDER_EDGES:
                if i < num_joints and j < num_joints:
                    adj[i, j] = True
                    adj[j, i] = True
    ...
    def forward(self, joint_tokens: Tensor) -> Tuple[Tensor, Tensor]:
        """
        Args:
            joint_tokens: [B, num_joints, joint_dim]

        Returns:
            refined_joints: [B, num_joints, joint_dim]
            attn_weights: [B, num_heads, num_joints, num_joints]
        """
        B, N, D = joint_tokens.shape

        # Q, K, V projections
        Q = self.q_proj(joint_tokens)  # [B, N, D]
        K = self.k_proj(joint_tokens)
        V = self.v_proj(joint_tokens)

        # Reshape for multi-head attention
        Q = Q.view(B, N, self.num_heads, self.head_dim).transpose(1, 2)  # [B, H, N, D/H]
        ...
        # Add learnable edge bias
        attn_scores = attn_scores + self.edge_bias.unsqueeze(0)  # [B, H, N, N]

        # Apply skeleton mask
        # adj_matrix: [N, N], True = can attend
        # We need to mask out positions where adj_matrix is False
        mask = ~self.adj_matrix  # [N, N], True = cannot attend
        ...
        attn_out = torch.matmul(attn_weights, V)  # [B, H, N, D/H]
...
class SkeletonGATLiftingNetwork(nn.Module):
    """
    Skeleton Graph Attention Network for 3D pose lifting.

    Pipeline:
    1. 2D coords → Joint embed
    2. Backbone Cross-Attention (depth query)
    3. HMD Cross-Attention (3D reference)
    4. Skeleton GAT × N (structural relations with skeleton prior)
    ...
    def __init__(
        ...
        self.backbone_cross_attn = BackboneCrossAttention(
            joint_dim=joint_dim,
        ...
        # Skeleton Graph Attention layers (replacing self-attention)
        self.gat_layers = nn.ModuleList([
            SkeletonGraphAttention(
                joint_dim=joint_dim,
    ...
    def forward(
        ...
        # Backbone cross-attention (depth query)
        joint_tokens, backbone_attn = self.backbone_cross_attn(joint_tokens, backbone_feat)
        ...
        for gat in self.gat_layers:
            joint_tokens, gat_attn = gat(joint_tokens)
        ...
        attn_info = {
            ...
            'hmd_attn': hmd_attn,
            'gat_attns': gat_attns  # List of [B, H, N, N] attention maps
        }
...
class CustomEgoposeSkeletonGATHead(BaseHead):
    """
    Skeleton Graph Attention Network Head for 3D Pose Estimation.

    Replaces fully-connected self-attention with skeleton-based GAT.
    Joints only attend to anatomically connected neighbors.

    Key differences from CustomEgoposeAttentionLiftingHead:
    - SkeletonGraphAttention instead of JointSelfAttention
    - Explicit skeleton structure prior
    ...
    Args:
        ...
        skeleton_edges: Custom skeleton edges (default: None, uses DEFAULT_SKELETON_EDGES)
        use_second_order: Include 2-hop neighbors in attention (default: True)
    """
    ...
    def __init__(
        ...
        # Heatmap encoder (for Z)
        self.encoder = HeatmapEncoder(num_classes=out_channels, output_size=64)
        ...
        # Skeleton GAT lifting network
        self.lifting_network = SkeletonGATLiftingNetwork(
    ...
    def forward_lifting(
        ...
        """Forward pass for skeleton GAT lifting."""
        # Extract 2D coordinates
        coords_2d, confidence = soft_argmax_2d(heatmaps)

        # Detach 2D coords (role separation)
        ...
        # Skeleton GAT lifting
        pose_3d, attn_info = self.lifting_network(
            coords_2d_lift, confidence_lift, backbone_feat, hmd_info
        )

        return pose_3d, coords_2d, confidence, attn_info
    ...
    def decode(
        ...
        # Lifting
        pose_3d, coords_2d, confidence, attn_info = self.forward_lifting(
            batch_heatmaps, backbone_feat, HMD_info.float()
        )

        # Encode Z for heatmap reconstruction
        z = self.encoder(batch_heatmaps.float())
