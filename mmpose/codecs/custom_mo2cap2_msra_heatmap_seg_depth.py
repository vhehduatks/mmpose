...
class Custom_mo2cap2_MSRAHeatmap_seg_depth(BaseKeypointCodec):
	...
	label_mapping_table = dict(
		...
		depthmap='depthmap',segmap='segmap'
		)

	auxiliary_encode_keys = {'keypoint3d','depth_path','seg_path'}
	##
	...
	def encode(self,
		...
		with Image.open(depth_path) as depth_img:
			depthmap = np.array(depth_img).astype(np.float32)  # Depth 이미지를 float32 배열로 변환

		# 2. Segmentation (GT) 불러오기
		with Image.open(seg_path) as seg_img:
			segmap = np.array(seg_img).astype(np.uint8)  # Segmentation 이미지를 uint8 배열로 변환
			# Segmentation 값을 0, 1, 2로 변환 (0: 배경, 1: 흰색 영역, 2: 검은색 이외 나머지)
			segmap = np.round(segmap / 127).astype(np.uint8)  # 127로 나눠서 반올림 -> 0, 1, 2
		depthmap = np.transpose(depthmap, (2, 0, 1))
		depthmap = depthmap[0]
		
		depthmap = depthmap[np.newaxis,:]
		segmap = segmap[np.newaxis,:]

		encoded = dict(heatmaps=heatmaps, keypoint_weights=keypoint_weights, depthmap=depthmap, segmap=segmap)
