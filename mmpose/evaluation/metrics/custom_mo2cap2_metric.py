...
class CustomMo2Cap2Metric(BaseMetric):
	...
	def __init__(self,
				...
				loss_3d: ConfigType = dict(type='MSELoss')
				 ) -> None:
	...
	def gt_to_coco_json(self, gt_dicts: Sequence[dict],
		...
		for gt_dict in gt_dicts:
			...
			for ann in gt_dict['raw_ann_info']:
				...
				annotation = dict(
					# id=ann['id'], # 주석(annotation)의 고유 식별자
					id=ann['image_id'],
...
#TODO : 이거 egostan evaluate compute_metrics 사용해서 배치당 에러 dict에 저장 (process에서 pred,gt 정리해서 result에 저장후 이걸 compute_metrics에서 계산함)
	def compute_metrics(self, results: list) -> Dict[str, float]:
		...
		loss_3d = self.loss_3d_module(pred_batch_3d_keypoints.to(torch.double),gt_batch_keypoint_3d.to(torch.double))
	...
	def _do_python_keypoint_eval(self, outfile_prefix: str) -> list:
		...
		"""
		res_file = f'{outfile_prefix}.keypoints.json'
		coco_det = self.coco.loadRes(res_file) # 여기서 self.coco 불러옴
		sigmas = self.dataset_meta['sigmas']
